...
describe("fraud-detection branches uncovered closure", () => {
  ...
  it("returns level=high for score 61-80", async () => {
    ...
    // total ~ 72 → high
    const returns = Array.from({ length: 6 }, () =>
      mkReturn({
        daysAgo: 60,
  ...
  it("rate factor: 80-tier when 7-9 total returns", async () => {
    const returns = Array.from({ length: 7 }, () =>
      mkReturn({
  ...
  it("timing factor: 50-tier when 30-50% late with 2+ late", async () => {
    ...
    findManyMock.mockResolvedValue([
      mkReturn({
        daysAgo: 1,
        orderDaysAgo: 28,
        items: [{ reasonCode: `a${Math.random()}`, price: "5" }],
      }),
      mkReturn({
        daysAgo: 2,
        orderDaysAgo: 28,
        items: [{ reasonCode: `b${Math.random()}`, price: "5" }],
      }),
      mkReturn({
        daysAgo: 3,
        orderDaysAgo: 5,
        items: [{ reasonCode: `c${Math.random()}`, price: "5" }],
      }),
      mkReturn({
        daysAgo: 4,
        orderDaysAgo: 5,
        items: [{ reasonCode: `d${Math.random()}`, price: "5" }],
      }),
      mkReturn({
        daysAgo: 5,
        orderDaysAgo: 5,
        items: [{ reasonCode: `e${Math.random()}`, price: "5" }],
      }),
    ]);
