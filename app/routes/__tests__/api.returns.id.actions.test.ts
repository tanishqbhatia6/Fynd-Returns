...
describe("method + id + body guards", () => {
  ...
  it("parses action from form with JSON field (json override)", async () => {
    prismaMock.shop.findUnique.mockResolvedValueOnce({ id: "shop-1", shopDomain: "store.myshopify.com", settings: {} });
    prismaMock.returnCase.findFirst.mockResolvedValueOnce(mkReturnCase());
    const res = await callAction(mkFormReq({ json: JSON.stringify({ action: "add_note", note: "from json field" }) }));
    expect(res.status).toBeGreaterThanOrEqual(300);
    expect(prismaMock.returnCase.update).toHaveBeenCalledWith(expect.objectContaining({
...
describe('action: "update_instructions"', () => {
  ...
  it("upserts defaultReturnInstructions on shop settings", async () => {
    await callAction(mkJsonReq({ action: "update_instructions", returnInstructions: "Bring receipt" }));
...
describe('action: "cancel_order" — validation', () => {
  ...
  it("400 when numeric order id can't be resolved via fetchOrderByOrderNumber", async () => {
    prismaMock.returnCase.findFirst.mockResolvedValueOnce(mkReturnCase({ shopifyOrderId: "not-gid-or-numeric", shopifyOrderName: "" }));
