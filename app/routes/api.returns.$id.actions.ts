...
import { enrichFyndError, classifyFyndError, enrichRefundError, isRedirectResponse, extractErrorMessage } from "../lib/return-action-errors.server";
import {
  handleAddNote,
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  if (actionType === "update_status" && newStatus) return await handleUpdateStatus(handlerCtx, handlerBody);

  if (actionType === "add_note") return await handleAddNote(handlerCtx, handlerBody);

  if (actionType === "save_notes_for_customer") return await handleSaveNotesForCustomer(handlerCtx, handlerBody);
  ...
  if (actionType === "update_instructions") return await handleUpdateInstructions(handlerCtx, handlerBody);

  // ── Approve customer cancellation request ──
  if (actionType === "approve_cancellation") return await handleApproveCancellation(handlerCtx, handlerBody);

  // ── Decline customer cancellation request ──
  if (actionType === "decline_cancellation") return await handleDeclineCancellation(handlerCtx, handlerBody);
