...
export const handleProcessReplacement: ReturnActionHandler = async (ctx) => {
  ...
  return await withSpan("return.action.process_replacement", {
    ...
    try {
      ...
      const replacementLineItems = (returnCase.items ?? [])
        ...
        .map((item) => {
          ...
          return {
            ...
            title: (item as { title?: string }).title || shopifyItem?.title || item.sku || "Replacement item",
            quantity: item.qty,
      ...
      try {
        const completeRes = await admin.graphql(DRAFT_ORDER_COMPLETE_MUTATION, { variables: { id: draftOrder.id } });
