/**
 * Tests for webhooks.orders.create.tsx — guarded webhook pattern.
 *
 * Each must:
...
import { action } from "../webhooks.orders.create";
...
describe("webhooks.orders.create", () => {
  ...
  it("returns 200 when no fynd affiliate id — no DB lookup", async () => {
    authenticateWebhookMock.mockResolvedValueOnce({
      shop: "store.myshopify.com",
      payload: {
