...
function tokenResponse(body: Record<string, unknown> = { access_token: "tok-extra", expires_in: 3600 }, status = 200) {
...
describe("createFyndClientOrError extra — missing client credentials", () => {
  ...
  it("accepts snake_case client_id/client_secret in credentials JSON", async () => {
    ...
    const res = await createFyndClientOrError({
      ...
      fyndEnvironment: "uat",
      fyndCredentials: JSON.stringify({ platform: { client_id: "cid-snake", client_secret: "sec-snake" } }),
    } as never, { requirePlatform: true });
...
describe("createFyndClientOrError extra — FDK fallback (raw-OAuth) path", () => {
  ...
  it("uses raw OAuth (not FDK) — token request hits /service/panel/authentication", async () => {
    ...
    const res = await createFyndClientOrError({
      ...
      fyndEnvironment: "uat",
      fyndCredentials: JSON.stringify({ platform: { clientId: "cid-fdk", clientSecret: "sec-fdk" } }),
    } as never, { requirePlatform: true });
    expect(res.ok).toBe(true);
  ...
  it("returns Fynd login failed error when OAuth returns 401", async () => {
    globalThis.fetch = vi.fn(async () => new Response("invalid_client", { status: 401 })) as typeof fetch;
    const { createFyndClientOrError } = await import("../fynd.server");
    const res = await createFyndClientOrError({
      fyndApplicationId: "app-fail-401",
      fyndCompanyId: "co-fail-401-uniq",
      fyndEnvironment: "uat",
      fyndCredentials: JSON.stringify({ platform: { clientId: "cid-401", clientSecret: "sec-401" } }),
    } as never, { requirePlatform: true });
    expect(res.ok).toBe(false);
  ...
  it("returns Fynd login failed error when OAuth body has no access_token", async () => {
    ...
    const res = await createFyndClientOrError({
      ...
      fyndEnvironment: "uat",
      fyndCredentials: JSON.stringify({ platform: { clientId: "cid-nt", clientSecret: "sec-nt" } }),
    } as never, { requirePlatform: true });
    expect(res.ok).toBe(false);
    if (!res.ok) expect(res.error).toMatch(/Fynd login failed/);
  });
  ...
  it("constructs client with companyId, applicationId and base URL from settings", async () => {
    ...
    const res = await createFyndClientOrError({
      ...
      fyndCredentials: JSON.stringify({ platform: { clientId: "cid-shape", clientSecret: "sec-shape" } }),
    } as never, { requirePlatform: true });
    expect(res.ok).toBe(true);
    if (res.ok) {
...
describe("createFyndClientOrError extra — cached token reuse", () => {
  ...
  it("second call with same baseUrl/companyId/clientId reuses cached token (single fetch)", async () => {
    const fetchSpy = vi.fn(async () => tokenResponse({ access_token: "cached-tok", expires_in: 3600 })) as unknown as typeof fetch;
    ...
    const settings = {
      ...
      fyndEnvironment: "uat",
      fyndCredentials: JSON.stringify({ platform: { clientId: "cid-cache-A", clientSecret: "sec-cache" } }),
    } as never;
  ...
  it("different companyId -> separate cache entry, two fetches", async () => {
    const fetchSpy = vi.fn(async () => tokenResponse({ access_token: "tok-x", expires_in: 3600 })) as unknown as typeof fetch;
    globalThis.fetch = fetchSpy;
    const { createFyndClientOrError } = await import("../fynd.server");
  ...
  it("different clientId -> separate cache entry, two fetches", async () => {
    const fetchSpy = vi.fn(async () => tokenResponse({ access_token: "tok-y", expires_in: 3600 })) as unknown as typeof fetch;
    globalThis.fetch = fetchSpy;
    const { createFyndClientOrError } = await import("../fynd.server");
    const baseSettings = {
      fyndApplicationId: "app",
  ...
  it("cached token reuse also applies on requirePlatform=false branch", async () => {
    const fetchSpy = vi.fn(async () => tokenResponse({ access_token: "tok-rp-false", expires_in: 3600 })) as unknown as typeof fetch;
    globalThis.fetch = fetchSpy;
    const { createFyndClientOrError } = await import("../fynd.server");
    const settings = {
...
describe("createFyndClientOrError extra — requirePlatform=false acceptance", () => {
  ...
  it("requirePlatform=false yields a Platform client (Storefront not used)", async () => {
    ...
    const res = await createFyndClientOrError({
      ...
      fyndEnvironment: "uat",
      fyndCredentials: JSON.stringify({ platform: { clientId: "cid-rpf", clientSecret: "sec-rpf" } }),
    } as never, { requirePlatform: false });
    expect(res.ok).toBe(true);
    if (res.ok) {
  ...
  it("requirePlatform=false propagates OAuth failure as Fynd login failed", async () => {
    globalThis.fetch = vi.fn(async () => new Response("server down", { status: 500 })) as typeof fetch;
    const { createFyndClientOrError } = await import("../fynd.server");
    const res = await createFyndClientOrError({
      fyndApplicationId: "app-rpf-500",
      fyndCompanyId: "co-rpf-500-uniq",
      fyndEnvironment: "uat",
      fyndCredentials: JSON.stringify({ platform: { clientId: "cid-500", clientSecret: "sec-500" } }),
    } as never, { requirePlatform: false });
    expect(res.ok).toBe(false);
    if (!res.ok) expect(res.error).toMatch(/Fynd login failed/);
  });
