...
describe("GET /api/integrations/gorgias (widget)", () => {
  ...
  it("401 + Unauthorized card when API key mismatches", async () => {
    ...
    const res = await loader({ request: mkReq("shop=x&api_key=wrong"), params: {}, context: {} } as never);
    ...
    prismaMock.returnCase.findMany.mockResolvedValueOnce([
      {
        ...
        customerName: "Jane", isGiftReturn: true, fraudRiskLevel: "high", fraudRiskScore: 82,
        items: [{ title: "T-shirt", qty: 2 }],
      },
    ]);
    const res = await loader({ request: mkReq("shop=x&email=jane@example.com"), params: {}, context: {} } as never);
    const html = await res.text();
  ...
  it("authenticates successfully when api key matches", async () => {
    ...
    prismaMock.returnCase.findMany.mockResolvedValueOnce([]);
    const res = await loader({ request: mkReq("shop=x&email=a@b.com&api_key=secret"), params: {}, context: {} } as never);
