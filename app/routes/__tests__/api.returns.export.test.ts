...
describe("GET /api/returns/export", () => {
  it("creates a shop record when one doesn't exist", async () => {
    ...
    const res = await loader({ request: mkReq("range=last_30_days"), params: {}, context: {} } as never);
  ...
  it("400 when row count exceeds MAX_EXPORT_ROWS (10000)", async () => {
    ...
    const res = await loader({ request: mkReq("range=last_year"), params: {}, context: {} } as never);
  ...
  it("emits CSV with UTF-8 BOM + CRLF line terminator", async () => {
    ...
    prismaMock.returnCase.findMany.mockResolvedValueOnce([{
      id: "rc-long-id", returnRequestNo: "R-1", shopifyOrderName: "#1001",
      status: "approved", resolutionType: "refund",
      customerName: "Jane", customerEmailNorm: "j@x.com", customerPhoneNorm: null,
      customerCity: null, customerCountry: null, customerAddress1: null,
      customerAddress2: null, customerProvince: null, customerZip: null, customerLandmark: null,
      fyndReturnNo: null, fyndReturnId: null, fyndShipmentId: null,
      returnAwb: null, forwardAwb: null, refundStatus: null,
      refundJson: null, createdAt: new Date(), updatedAt: new Date(),
      items: [{ sku: "SKU-1", title: "Item", qty: 2, price: "20.00", condition: "new", reasonCode: "defective" }],
      events: [],
    }]);
  ...
  it("anonymises PII when ?anonymize=true", async () => {
    ...
    prismaMock.returnCase.count.mockResolvedValueOnce(1);
    prismaMock.returnCase.findMany.mockResolvedValueOnce([{
      id: "rc-1", returnRequestNo: null, shopifyOrderName: "#1001",
      status: "approved", resolutionType: "refund",
      customerName: "Jane Doe", customerEmailNorm: "jane@example.com", customerPhoneNorm: "+14155551212",
      customerCity: "SF", customerCountry: "US",
      customerAddress1: "1 Main St", customerAddress2: null,
      customerProvince: "CA", customerZip: "94101", customerLandmark: null,
      fyndReturnNo: null, fyndReturnId: null, fyndShipmentId: null,
      returnAwb: null, forwardAwb: null, refundStatus: null,
      refundJson: null, createdAt: new Date(), updatedAt: new Date(),
      items: [], events: [],
  ...
  it("applies status + search filter to where clause", async () => {
    ...
    prismaMock.returnCase.count.mockResolvedValueOnce(0);
    prismaMock.returnCase.findMany.mockResolvedValueOnce([]);
    await loader({ request: mkReq("status=approved&query=1001"), params: {}, context: {} } as never);
  ...
  it("emits one row per item when return has multiple items", async () => {
    ...
    prismaMock.returnCase.count.mockResolvedValueOnce(1);
    prismaMock.returnCase.findMany.mockResolvedValueOnce([{
      id: "rc-1", returnRequestNo: "R-1", shopifyOrderName: "#1001", status: "approved",
      resolutionType: "refund", customerName: null, customerEmailNorm: null,
      customerPhoneNorm: null, customerCity: null, customerCountry: null,
      customerAddress1: null, customerAddress2: null, customerProvince: null,
      customerZip: null, customerLandmark: null,
      fyndReturnNo: null, fyndReturnId: null, fyndShipmentId: null,
      returnAwb: null, forwardAwb: null, refundStatus: null, refundJson: null,
  ...
  it("emits one empty-items row when return has no items", async () => {
    ...
    prismaMock.returnCase.count.mockResolvedValueOnce(1);
    prismaMock.returnCase.findMany.mockResolvedValueOnce([{
      id: "rc-1", returnRequestNo: "R-1", shopifyOrderName: "#1001", status: "pending",
      resolutionType: "refund", customerName: null, customerEmailNorm: null,
      customerPhoneNorm: null, customerCity: null, customerCountry: null,
      customerAddress1: null, customerAddress2: null, customerProvince: null,
      customerZip: null, customerLandmark: null,
  ...
  it("escapes commas and quotes in values", async () => {
    ...
    prismaMock.returnCase.findMany.mockResolvedValueOnce([{
      ...
      status: "pending", resolutionType: "refund",
      customerName: null, customerEmailNorm: null, customerPhoneNorm: null,
      customerCity: null, customerCountry: null, customerAddress1: null,
      customerAddress2: null, customerProvince: null, customerZip: null,
      customerLandmark: null, fyndReturnNo: null, fyndReturnId: null,
      fyndShipmentId: null, returnAwb: null, forwardAwb: null,
      refundStatus: null, refundJson: null,
      createdAt: new Date(), updatedAt: new Date(),
      items: [], events: [],
    }]);
    const res = await loader({ request: mkReq(), params: {}, context: {} } as never);
