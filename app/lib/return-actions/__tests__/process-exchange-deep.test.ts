...
const {
  ...
  createRefundMock: vi.fn<(...args: unknown[]) => Promise<unknown>>(async () => ({
    success: true, refundId: "gid://shopify/Refund/1", refundAmount: "3.00", refundCurrency: "USD", refundCreatedAt: new Date().toISOString(), refundMethod: "original",
  })),
      ...
      if (q.includes("draftOrderCreate")) {
        ...
        return {
          json: async () => ({
            data: {
              draftOrderCreate: {
                draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1", invoiceUrl: null, totalPrice: "0" },
                userErrors: [],
              },
            },
          }),
...
describe("handleProcessExchange — portalVariants extraction", () => {
  ...
  it("picks the newest event with a non-empty exchangeVariants array", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/V_NEW", { id: "gid://shopify/ProductVariant/V_NEW", price: "10.00", inventoryAvailable: 5, productTitle: "New", variantTitle: "Default Title", sku: "SKU-NEW" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-newest", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/V_NEW" }] }), happenedAt: new Date() },
      { id: "ev-older", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/V_OLD" }] }), happenedAt: new Date() },
    ] as never);
    const ctx = mkCtx();
    const adminMock = mkAdmin();
    ctx.admin = adminMock.admin;
    await expectRedirect(handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody), "/app/returns/rc-1");
    // Confirm we fetched the newest variant id only
    expect(fetchVariantInfoMock).toHaveBeenCalledWith(expect.anything(), ["gid://shopify/ProductVariant/V_NEW"]);
  });
  ...
  it("skips event whose exchangeVariants is an empty array (legacy fall-through)", async () => {
    fetchOrderMock.mockResolvedValueOnce(baseOrder());
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-empty", payloadJson: JSON.stringify({ exchangeVariants: [] }), happenedAt: new Date() },
    ] as never);
    const ctx = mkCtx();
    ctx.admin = mkAdmin().admin;
    await expectRedirect(handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody), "/app/returns/rc-1");
...
describe("handleProcessExchange — variant info fetching", () => {
    ...
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      {
        id: "ev-1",
        payloadJson: JSON.stringify({
          exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/MISSING", variantTitle: "Custom Title" }],
        }),
        happenedAt: new Date(),
      },
    ] as never);
  ...
  it("matches by index when portalVariants length matches returnedItems count and lineItemId is missing", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/IDX", { id: "gid://shopify/ProductVariant/IDX", price: "11.00", inventoryAvailable: 5, productTitle: "Idx Match", variantTitle: "Default Title", sku: "SKU-IDX" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      {
        id: "ev-1",
  ...
  it("strips ' — Default Title' suffix from replacement title", async () => {
    fetchOrderMock.mockResolvedValueOnce({
      id: "gid://shopify/Order/1", email: "u@example.com", currencyCode: "USD",
      lineItems: [{ id: "gid://shopify/LineItem/1", title: "Item 1", sku: "SKU-1", price: "10.00", quantity: 1 }],
    });
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/X", { id: "gid://shopify/ProductVariant/X", price: "10.00", inventoryAvailable: 5, productTitle: "Cool Tee", variantTitle: "Default Title", sku: "SKU-X" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-1", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/X" }] }), happenedAt: new Date() },
    ] as never);
    const ctx = mkCtx();
    ctx.admin = mkAdmin().admin;
    await expectRedirect(handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody), "/app/returns/rc-1");
...
describe("handleProcessExchange — multi-line price diff", () => {
  it("sums mixed up/down deltas across lines (qty>1) producing positive net diff", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/A2", { id: "gid://shopify/ProductVariant/A2", price: "12.00", inventoryAvailable: 10, productTitle: "A bigger", variantTitle: "L", sku: "A2" }],
      ["gid://shopify/ProductVariant/B2", { id: "gid://shopify/ProductVariant/B2", price: "15.00", inventoryAvailable: 10, productTitle: "B smaller", variantTitle: "S", sku: "B2" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      {
        id: "ev-1",
        payloadJson: JSON.stringify({
          exchangeVariants: [
            { lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/A2" },
            { lineItemId: "gid://shopify/LineItem/2", variantId: "gid://shopify/ProductVariant/B2" },
          ],
        }),
        happenedAt: new Date(),
      },
    ...
    const ctx = mkCtx({
      returnCase: {
        ...mkCtx().returnCase,
        items: [
          { id: "li-1", shopifyLineItemId: "gid://shopify/LineItem/1", qty: 2, sku: "A1", price: "10.00", reasonCode: null, notes: null, title: "A" },
          { id: "li-2", shopifyLineItemId: "gid://shopify/LineItem/2", qty: 1, sku: "B1", price: "20.00", reasonCode: null, notes: null, title: "B" },
        ],
      } as never,
    });
    ctx.admin = mkAdmin().admin;
  ...
  it("treats non-numeric replacement price as 0 in subtotal sum", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/V", { id: "gid://shopify/ProductVariant/V", price: "not-a-number", inventoryAvailable: 5, productTitle: "P", variantTitle: "L", sku: "V" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-1", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/V" }] }), happenedAt: new Date() },
    ] as never);
    const ctx = mkCtx();
    ctx.admin = mkAdmin().admin;
    await expectRedirect(handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody), "/app/returns/rc-1");
  ...
  it("price-diff exactly 0 → completed_free flow (no refund call)", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/SAME", { id: "gid://shopify/ProductVariant/SAME", price: "10.00", inventoryAvailable: 5, productTitle: "Same", variantTitle: "Default Title", sku: "S1" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-1", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/SAME" }] }), happenedAt: new Date() },
    ] as never);
    const ctx = mkCtx();
    ctx.admin = mkAdmin().admin;
    await expectRedirect(handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody), "/app/returns/rc-1");
...
describe("handleProcessExchange — completed_with_refund branch", () => {
  function setupRefundCtx(): ReturnHandlerContext {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/CHEAP", { id: "gid://shopify/ProductVariant/CHEAP", price: "7.00", inventoryAvailable: 5, productTitle: "Cheap", variantTitle: "S", sku: "C1" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-1", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/CHEAP" }] }), happenedAt: new Date() },
    ] as never);
    const ctx = mkCtx();
    ctx.admin = mkAdmin().admin;
    return ctx;
...
describe("handleProcessExchange — draftOrderComplete error handling", () => {
  function setupCompletedFreeCtx(opts: {
    ...
    if (opts.completeThrow) {
      ctx.admin = {
        graphql: vi.fn(async (q: string) => {
          if (q.includes("draftOrderCreate")) {
            return { json: async () => ({ data: { draftOrderCreate: { draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1" }, userErrors: [] } } }) };
          }
  ...
  it("falls back to draft id/name when complete returns no order", async () => {
    const ctx = setupCompletedFreeCtx({
      completeOverride: async () => ({
        data: { draftOrderComplete: { draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1", order: null }, userErrors: [] } },
      }),
    });
    await expectRedirect(handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody), "/app/returns/rc-1");
    const update = prismaMock.returnCase.update.mock.calls[0][0];
...
describe("handleProcessExchange — Fynd transition push", () => {
  ...
  it("writes fynd_exchange_sync_failed event when updateShipmentStatus throws", async () => {
    const updateShipmentStatus = vi.fn(async () => { throw new Error("Fynd down"); });
    createFyndClientOrErrorMock.mockResolvedValueOnce({
      ok: true,
      client: { updateShipmentStatus, getShipments: vi.fn() },
    });
...
describe("handleProcessExchange — customer notification copy", () => {
  it("invoice_pending: notifies with payment-link copy", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/UP", { id: "gid://shopify/ProductVariant/UP", price: "15.00", inventoryAvailable: 5, productTitle: "Up", variantTitle: "L", sku: "U1" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-1", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/UP" }] }), happenedAt: new Date() },
    ] as never);
    const ctx = mkCtx();
    ctx.admin = mkAdmin().admin;
    await expectRedirect(handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody), "/app/returns/rc-1");
  ...
  it("completed_with_refund: notifies with refund copy", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/DOWN", { id: "gid://shopify/ProductVariant/DOWN", price: "7.00", inventoryAvailable: 5, productTitle: "Down", variantTitle: "S", sku: "D1" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-1", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/DOWN" }] }), happenedAt: new Date() },
    ] as never);
    const ctx = mkCtx();
    ctx.admin = mkAdmin().admin;
    await expectRedirect(handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody), "/app/returns/rc-1");
