...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if ((s as { emailTemplatesJson?: string | null } | null)?.emailTemplatesJson) {
    try {
      emailTemplatesJson = JSON.parse((s as { emailTemplatesJson: string }).emailTemplatesJson);
    } catch {
      /* ignore */
    }
  }
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (intent === "save_email_templates") {
    ...
    try {
      ...
      return {
        success: false,
        error: e instanceof Error ? e.message : "Failed to save templates.",
      };
    }
  }
  ...
  const resolvedPass: string | null =
    submittedPass === ""
      ? null
    ...
    >
      <div
        style={{
          ...
          background: checked
            ? (accentColor || "var(--rpm-accent)") + "14"
            : "var(--rpm-surface-elevated)",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, fontSize: 14, color: "var(--rpm-text)", marginBottom: 2 }}>
          {label}
        </div>
        <div style={{ fontSize: 12, color: "var(--rpm-text-muted)", lineHeight: 1.5 }}>
          {description}
        </div>
      </div>
      <label
    ...
    <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 18 }}>
      <div
        style={{
          ...
          height: 40,
          borderRadius: "var(--rpm-radius)",
          background: "var(--rpm-accent-subtle)",
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: "var(--rpm-text)" }}>
            {title}
          </h2>
          {badge}
        </div>
    ...
    <div style={{ flex: half ? "1 1 45%" : "1 1 100%", minWidth: half ? 180 : undefined }}>
      <label
        style={{
          ...
          fontSize: 13,
          fontWeight: 600,
          color: "var(--rpm-text-secondary)",
          marginBottom: 6,
        }}
      >
        {label}
        {required && <span style={{ color: "var(--rpm-danger)" }}>*</span>}
      </label>
      <input
        type={type}
        name={name}
      ...
      />
      {helpText && (
        <p style={{ margin: "4px 0 0", fontSize: 11, color: "var(--rpm-text-subtle)" }}>
          {helpText}
        </p>
      )}
    </div>
...
export default function Notifications() {
  ...
  /* v8 ignore start */
  const [emailTemplates, setEmailTemplates] = useState<Record<string, TemplateData>>(
    data.emailTemplatesJson ?? {},
  );
  /* v8 ignore stop */
  ...
  const saveTemplate = useCallback(
    (key: string) => {
      const updated = {
        ...emailTemplates,
        [key]: { subject: templateSubject, bodyHtml: templateBody },
      };
      ...
      <div className="app-content layout-medium">
          ...
          >
                ...
                smtpFilled ? (
                  <span
                    style={{
                      ...
                      padding: "2px 8px",
                      borderRadius: 12,
                      background: data.smtpConfigured ? "#ECFDF5" : "#FEF3C7",
                      color: data.smtpConfigured ? "#059669" : "#D97706",
                    }}
                  >
            ...
            <div
              style={{
                ...
                border: "1px solid var(--rpm-border-color)",
                borderRadius: "var(--rpm-radius)",
                padding: "14px 16px",
                marginBottom: 20,
                fontSize: 13,
            ...
            >
              <strong style={{ color: "var(--rpm-text-secondary)" }}>Common SMTP providers:</strong>{" "}
              Gmail (smtp.gmail.com:587), Outlook (smtp-mail.outlook.com:587), SendGrid
              (smtp.sendgrid.net:587), AWS SES, or any custom SMTP server. For Gmail, use an App
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14 }}>
              ...
              <Field
                ...
                half
                helpText="Defaults to username if empty"
              />
              <Field
                label="From Name"
                name="smtpFromName"
                ...
                half
                helpText='Shows as "From Name" in emails'
              />
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 16 }}>
              ...
              <button
                ...
                disabled={!smtpFilled || testFetcher.state !== "idle"}
                style={{
                  padding: "8px 16px",
                  fontSize: 13,
                  fontWeight: 600,
                  borderRadius: "var(--rpm-radius-sm)",
                  border: "var(--rpm-border-strong)",
                  background: "var(--rpm-surface)",
                  color: "var(--rpm-text-secondary)",
                  cursor: smtpFilled ? "pointer" : "not-allowed",
                  opacity: smtpFilled ? 1 : 0.5,
                  display: "flex",
                  alignItems: "center",
                  gap: 6,
                  transition: "var(--rpm-transition)",
            ...
            {testResult && (
              ...
              >
                {testResult.success
                  ? "SMTP connection successful — emails are ready to send."
          ...
          >
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12, color: "var(--rpm-text-muted)", lineHeight: 1.5 }}>
                  Play a notification sound when new returns arrive in the admin panel.
                </div>
              </div>
              <button
                type="button"
              ...
              >
                <input
                  ...
                  checked={adminSound}
                  onChange={(e) => setAdminSound(e.target.checked)}
                  style={{ position: "absolute", opacity: 0, width: 0, height: 0 }}
                />
                <span
                  style={{
              ...
              {Object.entries(previewTemplates).map(([key, t]) => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12,
                    fontWeight: 600,
                    borderRadius: "var(--rpm-radius-full)",
            ...
            {previewTemplate && previewTemplates[previewTemplate] && (
              ...
              >
                <div
                  style={{
                    padding: "8px 14px",
                    background: previewTemplates[previewTemplate].color + "10",
                    borderBottom: `1px solid ${previewTemplates[previewTemplate].color}22`,
                    display: "flex",
                    alignItems: "center",
                    gap: 8,
                  }}
                >
                  <div
                    style={{
                      ...
                      height: 8,
                      borderRadius: "50%",
                      background: previewTemplates[previewTemplate].color,
                    }}
                  />
                  <span
                    style={{
                      fontSize: 12,
                      fontWeight: 600,
                      color: previewTemplates[previewTemplate].color,
                    }}
                  >
                    {previewTemplates[previewTemplate].label}
                  </span>
                  <span
                    style={{ fontSize: 11, color: "var(--rpm-text-muted)", marginLeft: "auto" }}
                  >
                    Preview
                  </span>
                </div>
                ...
                >
                  ...
                  <div
                    dangerouslySetInnerHTML={{ __html: previewTemplates[previewTemplate].preview }}
                  />
                  <div
                    style={{
                      padding: "12px 24px",
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <span style={{ fontWeight: 600, fontSize: 14, color: "var(--rpm-text)" }}>
                          {evt.label}
                        </span>
                        {hasCustom && (
                          <span
                        ...
                        >
                          ...
                          <button
                            ...
                            style={{
                              ...
                              color: "#fff",
                              cursor: "pointer",
                              opacity: templateFetcher.state !== "idle" ? 0.7 : 1,
                            }}
                          >
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 12, color: "var(--rpm-text-muted)" }}>
                  Send real-time WhatsApp messages to customers on key events
                </div>
              </div>
            ...
            {waEnabled && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div>
                  ...
                  >
                    API Key / Access Token <span style={{ color: "#d72c0d" }}>*</span>
                  </label>
                  <input
                    type="password"
                    name="whatsappApiKey"
                ...
                {waProvider === "meta_cloud" && (
                  <div>
                    ...
                    >
                      Phone Number ID <span style={{ color: "#d72c0d" }}>*</span>
                    </label>
                    <input
                      type="text"
                    ...
                    />
                    <div style={{ fontSize: 11, color: "var(--rpm-text-muted)", marginTop: 3 }}>
                ...
                >
                  WhatsApp messages will be sent to the customer's phone number on: return approved,
                  rejected, and refunded events.
                </div>
              </div>
            )}
            {!waEnabled && (
        ...
        >
          ...
          >
            <select
              name="logChannel"
              defaultValue={data.notificationLogFilters?.logChannel ?? ""}
              style={{
                fontSize: 12,
                padding: "5px 8px",
                borderRadius: 6,
                border: "1px solid var(--rpm-border-color, #e5e7eb)",
              }}
            ...
            <select
              name="logStatus"
              defaultValue={data.notificationLogFilters?.logStatus ?? ""}
              style={{
                fontSize: 12,
                padding: "5px 8px",
                borderRadius: 6,
            ...
            {(data.notificationLogFilters?.logChannel ||
              data.notificationLogFilters?.logStatus ||
              data.notificationLogFilters?.logQ) && (
              <a
                href="?"
                style={{
                  fontSize: 12,
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                ...
                <tbody>
                  {data.notificationLogs.map((log) => (
                    <tr
                      key={log.id}
                      style={{ borderBottom: "1px solid var(--rpm-border-color, #f1f5f9)" }}
                    >
                      <td
                        style={{
                          padding: "8px 10px",
