...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  return {
    ...
    noReturnPeriodStart: s?.noReturnPeriodStart
      ? new Date(s.noReturnPeriodStart).toISOString().slice(0, 10)
      : "",
    noReturnPeriodEnd: s?.noReturnPeriodEnd
      ? new Date(s.noReturnPeriodEnd).toISOString().slice(0, 10)
      : "",
    ...
    portalAllowedFulfillmentStatuses: (() => {
      try {
        return s?.portalAllowedFulfillmentStatuses
          ? (JSON.parse(s.portalAllowedFulfillmentStatuses) as string[])
          : ["FULFILLED", "PARTIALLY_FULFILLED"];
      } catch {
        return ["FULFILLED", "PARTIALLY_FULFILLED"];
      }
    })(),
    ...
    allowedFyndStatusesForRefund: (() => {
      try {
        return s?.allowedFyndStatusesForRefund
          ? (JSON.parse(s.allowedFyndStatusesForRefund) as string[])
          : [];
      } catch {
        return [];
      }
    ...
    refundGatePreset: (() => {
      ...
      try {
        const statuses = s?.allowedFyndStatusesForRefund
          ? (JSON.parse(s.allowedFyndStatusesForRefund) as string[])
          : [];
    ...
    allowedFyndStatusesForReturn: (() => {
      try {
        return s?.allowedFyndStatusesForReturn
          ? (JSON.parse(s.allowedFyndStatusesForReturn) as string[])
          : [];
      } catch {
        return [];
      }
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const returnFeeAmount = Math.max(
    0,
    parseFloat(String(formData.get("returnFeeAmount") ?? "0")) || 0,
  );
  ...
  const refundStoreCreditPct = Math.min(
    100,
    Math.max(0, parseInt(String(formData.get("refundStoreCreditPct") ?? "100"), 10) || 100),
  );
  const discountCodeRefundEnabled = formData.get("discountCodeRefundEnabled") === "on";
  const discountCodePrefix =
    (formData.get("discountCodePrefix") as string | null)?.trim() || "RETURN";
  const discountCodeExpiryDays = Math.max(
    1,
    parseInt(String(formData.get("discountCodeExpiryDays") ?? "90"), 10) || 90,
  );
  const portalExchangeEnabled = formData.get("portalExchangeEnabled") === "on";
  const portalAllowedFulfillmentStatusesRaw = formData.getAll(
    "portalAllowedFulfillmentStatuses",
  ) as string[];
  const portalAllowedFulfillmentStatuses =
    portalAllowedFulfillmentStatusesRaw.length > 0
  ...
  const fyndConsolidateWindowHours = [1, 4, 8, 24].includes(
    parseInt(String(formData.get("fyndConsolidateWindowHours") ?? "4"), 10),
  )
  ...
  const allowedFyndStatusesForReturnRaw = formData.getAll(
    "allowedFyndStatusesForReturn",
  ) as string[];
  const allowedFyndStatusesForReturn =
    allowedFyndStatusesForReturnRaw.length > 0
      ? JSON.stringify(
          allowedFyndStatusesForReturnRaw.map((s) => s.trim().toLowerCase()).filter(Boolean),
        )
  ...
  const scheduledReportDay = Math.max(
    1,
    Math.min(28, parseInt(String(formData.get("scheduledReportDay") ?? "1"), 10) || 1),
  );
  ...
  const greenReturnsDonateMessage =
    ((formData.get("greenReturnsDonateMessage") as string) ?? "").trim() || null;
  ...
  const noStart =
    noReturnPeriodEnabled && noReturnPeriodStart && noReturnPeriodStart.trim()
      ? new Date(noReturnPeriodStart)
      : null;
  const noEnd =
    noReturnPeriodEnabled && noReturnPeriodEnd && noReturnPeriodEnd.trim()
      ? new Date(noReturnPeriodEnd)
      : null;
...
export default function ReturnSettings() {
  ...
  const [locationMode, setLocationMode] = React.useState<"auto" | "manual">(
    data.refundLocationMode === "manual" ? "manual" : "auto",
  );
  ...
  const [paymentMethod, setPaymentMethod] = React.useState<"original" | "store_credit" | "both">(
    (["original", "store_credit", "both"].includes(data.refundPaymentMethod)
  ...
  const [portalExchangeEnabled, setPortalExchangeEnabled] = React.useState(
    data.portalExchangeEnabled,
  );
  const [allowedFulfillStatuses, setAllowedFulfillStatuses] = React.useState<string[]>(
    data.portalAllowedFulfillmentStatuses,
  );
  const [fyndConsolidateReturns, setFyndConsolidateReturns] = React.useState(
    data.fyndConsolidateReturns,
  );
  const [syncRefundToFynd, setSyncRefundToFynd] = React.useState(data.syncRefundToFynd);
  const [fyndConsolidateWindowHours, setFyndConsolidateWindowHours] = React.useState(
    data.fyndConsolidateWindowHours,
  );
  const [allowedFyndStatuses, setAllowedFyndStatuses] = React.useState<string[]>(
    data.allowedFyndStatusesForRefund,
  );
  const [fyndStatusGateEnabled, setFyndStatusGateEnabled] = React.useState(
    data.refundGatePreset !== "none" && data.refundGatePreset !== null,
  );
  const [refundGatePreset, setRefundGatePreset] = React.useState<RefundGatePreset>(
    (data.refundGatePreset ?? "none") as RefundGatePreset,
  );
  const [allowedFyndReturnStatuses, setAllowedFyndReturnStatuses] = React.useState<string[]>(
    data.allowedFyndStatusesForReturn,
  );
  const [fyndReturnGateEnabled, setFyndReturnGateEnabled] = React.useState(
    data.allowedFyndStatusesForReturn.length > 0,
  );

  // Scheduled reports
  const [scheduledReportEnabled, setScheduledReportEnabled] = React.useState(
    data.scheduledReportEnabled,
  );
  const [scheduledReportFrequency, setScheduledReportFrequency] = React.useState(
    data.scheduledReportFrequency,
  );
  const [scheduledReportDay, setScheduledReportDay] = React.useState(data.scheduledReportDay);
  const [scheduledReportEmails, setScheduledReportEmails] = React.useState(
    data.scheduledReportEmails,
  );
  // Gift returns
  const [giftReturnsEnabled, setGiftReturnsEnabled] = React.useState(data.giftReturnsEnabled);
  // Green returns donate
  const [greenReturnsDonateEnabled, setGreenReturnsDonateEnabled] = React.useState(
    data.greenReturnsDonateEnabled,
  );
  const [greenReturnsDonateMessage, setGreenReturnsDonateMessage] = React.useState(
    data.greenReturnsDonateMessage,
  );
  ...
  const [ridBodyMode, setRidBodyMode] = React.useState<ReturnIdBodyMode>(
    data.returnIdConfig.bodyMode,
  );
  ...
  const ridPreview = React.useMemo(
    () =>
      previewReturnRequestId({
        prefix: ridPrefix,
        separator: ridSeparator,
        bodyMode: ridBodyMode,
        hashLength: ridHashLength,
        sequentialPadding: ridSeqPadding,
        suffix: ridSuffix,
      }),
  ...
  React.useEffect(() => {
    ...
    data.restrictedProductTags,
    data.refundLocationMode,
    data.refundLocationId,
    data.refundPaymentMethod,
    data.refundStoreCreditPct,
    data.photoRequired,
    data.autoApproveEnabled,
    data.autoRefundEnabled,
    data.discountCodeRefundEnabled,
    data.discountCodePrefix,
    data.discountCodeExpiryDays,
          ...
          >
            ...
            <s-section>
              <div style={{ marginBottom: 12 }}>
                ...
                <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 12 }}>
                  During a specified promotional or sale event, returns for items purchased within
                  that period will not be processed. Note the date range.
                </p>
                <label style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  <input
                    type="checkbox"
                    name="noReturnPeriodEnabled"
                    checked={noReturnEnabled}
                    onChange={(e) => setNoReturnEnabled(e.target.checked)}
                  />
                ...
                {noReturnEnabled ? (
                  <div style={{ padding: 16, background: "#f6f6f7", borderRadius: 8 }}>
                    <p style={{ fontSize: 13, marginBottom: 8 }}>
                      Set the date range during which returns will not be accepted.
                    </p>
                  ...
                  <p style={{ fontSize: 13, color: "#6d7175" }}>
                    Enable the toggle above to configure a no-return date range.
                  </p>
                )}
              </div>
            </s-section>
            ...
            <s-section>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                ...
                />
                <s-button type="button" variant="secondary" onClick={addTag}>
                  Add
                </s-button>
              </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#6d7175" }}>
                      Configure how return request IDs are generated
                    </div>
                  </div>
                </div>
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 120 }}>
                      ...
                      <input
                        ...
                        maxLength={20}
                        onChange={(e) =>
                          setRidPrefix(e.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g, ""))
                        }
                        style={{
                          padding: "7px 10px",
                          borderRadius: 6,
                    ...
                    <div style={{ flex: 1, minWidth: 120 }}>
                      ...
                      <input
                        ...
                        maxLength={10}
                        onChange={(e) =>
                          setRidSuffix(e.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g, ""))
                        }
                        style={{
                          padding: "7px 10px",
                          borderRadius: 6,
                        ...
                        [
                          {
                            value: "hash" as const,
                            label: "Hash from ID",
                            desc: "Random alphanumeric (e.g. A1B2C3D4)",
                          },
                          {
                            value: "sequential" as const,
                            label: "Sequential",
                            desc: "Auto-incrementing counter (e.g. 000042)",
                          },
                          {
                            value: "date_hash" as const,
                            label: "Date + Hash",
                            desc: "Date prefix + hash (e.g. 260312-A1B2)",
                          },
                          {
                            value: "date_sequential" as const,
                            label: "Date + Sequential",
                            desc: "Date prefix + counter (e.g. 260312-042)",
                          },
                        ] as const
                      ).map((opt) => (
                        <label
            ...
            <s-section>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 10,
                      background: photoRequired ? "#EFF6FF" : "#F3F4F6",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke={photoRequired ? "#3B82F6" : "#9CA3AF"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                ...
                >
                  <input
                    aria-label="Photo required for returns"
                    type="checkbox"
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: 12,
                      transition: "all 0.15s",
                      background: photoRequired ? "#3B82F6" : "#cbd5e1",
                    }}
                  >
                    <span
                      style={{
                        position: "absolute",
                        left: photoRequired ? 22 : 2,
                        top: 2,
                        width: 20,
                        height: 20,
                        borderRadius: 10,
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 10 }}>
                Require customers to upload item photos for better assistance with return inquiries
                or concerns.
              </p>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span
                  style={{
                    ...
                    height: 8,
                    borderRadius: 4,
                    background: photoRequired ? "#22C55E" : "#D1D5DB",
                    transition: "background 0.15s",
                  }}
                />
                <span
                  style={{
                    fontSize: 12,
                    fontWeight: 500,
                    color: photoRequired ? "#15803D" : "#6B7280",
            ...
            <s-section>
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 12 }}>
                Configure a return fee that is subtracted from the refund amount automatically
                during the return process.
              </p>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <select
                  aria-label="Return fee currency"
                  name="returnFeeCurrency"
                  defaultValue={data.returnFeeCurrency}
                  style={{ padding: 10, borderRadius: 6, border: "1px solid #e1e3e5" }}
                >
            ...
            <s-section>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 10,
                      background: autoApproveEnabled ? "#F0FDF4" : "#F3F4F6",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      transition: "background 0.15s",
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke={autoApproveEnabled ? "#22C55E" : "#9CA3AF"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      style={{ transition: "stroke 0.15s" }}
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: 12,
                      transition: "all 0.15s",
                      background: autoApproveEnabled ? "#3B82F6" : "#cbd5e1",
                    }}
                  >
                    <span
                      style={{
                        position: "absolute",
                        left: autoApproveEnabled ? 22 : 2,
                        top: 2,
                        width: 20,
                        height: 20,
                        borderRadius: 10,
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 10 }}>
                You have the flexibility to either manually approve return requests or opt for an
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span
                  style={{
                    ...
                    height: 8,
                    borderRadius: 4,
                    background: autoApproveEnabled ? "#22C55E" : "#D1D5DB",
                    transition: "background 0.15s",
                  }}
                />
                <span
                  style={{
                    fontSize: 12,
                    fontWeight: 500,
                    color: autoApproveEnabled ? "#15803D" : "#6B7280",
                    transition: "color 0.15s",
                  }}
                >
            ...
            <s-section>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 10,
                      background: autoRefundEnabled ? "#FFFBEB" : "#F3F4F6",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      transition: "background 0.15s",
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke={autoRefundEnabled ? "#F59E0B" : "#9CA3AF"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      style={{ transition: "stroke 0.15s" }}
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: 12,
                      transition: "all 0.15s",
                      background: autoRefundEnabled ? "#3B82F6" : "#cbd5e1",
                    }}
                  >
                    <span
                      style={{
                        position: "absolute",
                        left: autoRefundEnabled ? 22 : 2,
                        top: 2,
                        width: 20,
                        height: 20,
                        borderRadius: 10,
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 10 }}>
                When Fynd generates a credit note, automatically trigger a Shopify refund. If
                disabled, refunds must be processed manually from the return detail page.
              </p>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span
                  style={{
                    ...
                    height: 8,
                    borderRadius: 4,
                    background: autoRefundEnabled ? "#22C55E" : "#D1D5DB",
                    transition: "background 0.15s",
                  }}
                />
                <span
                  style={{
                    fontSize: 12,
                    fontWeight: 500,
                    color: autoRefundEnabled ? "#15803D" : "#6B7280",
                    transition: "color 0.15s",
                  }}
                >
              ...
              <input
                ...
                value={autoRefundEnabled ? "on" : "off"}
              />
            </s-section>
            ...
            <s-section>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 10,
                      background: syncRefundToFynd ? "#EDE9FE" : "#F3F4F6",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      transition: "background 0.15s",
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke={syncRefundToFynd ? "#7C3AED" : "#9CA3AF"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      style={{ transition: "stroke 0.15s" }}
                ...
                >
                  <input
                    aria-label="Sync refund to Fynd"
                    type="checkbox"
                    checked={syncRefundToFynd}
                    onChange={(e) => setSyncRefundToFynd(e.target.checked)}
                    style={{ position: "absolute", opacity: 0, width: 0, height: 0 }}
                  />
                  <span
                    style={{
                      ...
                      borderRadius: 12,
                      transition: "all 0.15s",
                      background: syncRefundToFynd ? "#3B82F6" : "#cbd5e1",
                    }}
                  >
                    <span
                      style={{
                        position: "absolute",
                        left: syncRefundToFynd ? 22 : 2,
                        top: 2,
                        width: 20,
                        height: 20,
                        borderRadius: 10,
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 10 }}>
                After a refund is processed on Shopify (via admin or auto-refund), push{" "}
                <strong>credit_note_generated</strong> status back to Fynd. This keeps both
                platforms in sync when refunds are initiated from the Shopify side.
              </p>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span
                  style={{
                    ...
                    height: 8,
                    borderRadius: 4,
                    background: syncRefundToFynd ? "#22C55E" : "#D1D5DB",
                    transition: "background 0.15s",
                  }}
                />
                <span
                  style={{
                    fontSize: 12,
                    fontWeight: 500,
                    color: syncRefundToFynd ? "#15803D" : "#6B7280",
                    transition: "color 0.15s",
                  }}
                >
              ...
              <input
                type="hidden"
                name="syncRefundToFynd"
                value={syncRefundToFynd ? "on" : "off"}
              />
            </s-section>
            ...
            <s-section>
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 16 }}>
                Control how refunds are issued in Shopify. This applies to all refund paths — manual
                refunds, auto-refund on credit note, and webhook-triggered refunds.
              </p>
              <div
                style={{
                  display: "flex",
                  flexDirection: "column",
                  gap: 10,
                  marginBottom: paymentMethod === "both" ? 16 : 0,
                }}
              >
                <label
                  style={{
                    ...
                    borderRadius: 10,
                    border:
                      paymentMethod === "original" ? "2px solid #3B82F6" : "1px solid #E5E7EB",
                    transition: "all 0.15s",
                  }}
                >
                  <input
                    type="radio"
                    checked={paymentMethod === "original"}
                    onChange={() => setPaymentMethod("original")}
                    style={{ marginTop: 3 }}
                  />
                  <div style={{ flex: 1 }}>
                    <div
                    ...
                    <div style={{ fontSize: 12, color: "#6B7280", marginTop: 4, lineHeight: 1.5 }}>
                      Refund is returned to the customer's original payment method (credit card,
                      UPI, etc.). This is the standard Shopify refund behavior.
                    </div>
                  </div>
                </label>
                <label
                  style={{
                    ...
                    borderRadius: 10,
                    border:
                      paymentMethod === "store_credit" ? "2px solid #22C55E" : "1px solid #E5E7EB",
                    transition: "all 0.15s",
                  }}
                >
                  <input
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#6B7280", marginTop: 4, lineHeight: 1.5 }}>
                      Full refund amount is issued as store credit to the customer's account.
                      Requires new customer accounts to be enabled in your Shopify store.
                    </div>
                  </div>
                </label>
                <label
                  style={{
                    ...
                    borderRadius: 10,
                    border: paymentMethod === "both" ? "2px solid #F59E0B" : "1px solid #E5E7EB",
                    transition: "all 0.15s",
                  }}
                >
                  <input
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#6B7280", marginTop: 4, lineHeight: 1.5 }}>
                      Split the refund between the original payment method and store credit.
                      Configure the percentage allocated to store credit below.
                    </div>
                  </div>
                </label>
              </div>
              {paymentMethod === "both" && (
                ...
                >
                  <label
                    style={{ fontWeight: 600, fontSize: 13, display: "block", marginBottom: 10 }}
                  >
              ...
              {paymentMethod === "store_credit" && (
                ...
                >
                  ...
                  <span>
                    Store credit requires new customer accounts to be enabled in Shopify Settings.
                    The order must also be associated with a customer.
                  </span>
                </div>
              )}
            ...
            <s-section>
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 12 }}>
                ...
                Choose whether to automatically use the order's fulfillment location or manually
                select it each time.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
                <label
                  style={{
                    ...
                    cursor: "pointer",
                    padding: 12,
                    background: locationMode === "auto" ? "#EFF6FF" : "#F9FAFB",
                    borderRadius: 8,
                    border: locationMode === "auto" ? "2px solid #3B82F6" : "1px solid #E5E7EB",
                    transition: "all 0.15s",
                  }}
                >
                  <input
                    type="radio"
                    name="refundLocationMode"
                    value="auto"
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 13 }}>
                      Automatic — use fulfillment location
                    </div>
                    <div style={{ fontSize: 12, color: "#6B7280", marginTop: 2 }}>
                      Inventory will be restocked at the location the order was originally fulfilled
                      from. If unavailable, falls back to the default location below.
                    </div>
                  </div>
                </label>
                <label
                  style={{
                    ...
                    cursor: "pointer",
                    padding: 12,
                    background: locationMode === "manual" ? "#EFF6FF" : "#F9FAFB",
                    borderRadius: 8,
                    border: locationMode === "manual" ? "2px solid #3B82F6" : "1px solid #E5E7EB",
                    transition: "all 0.15s",
                  }}
                >
                  <input
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 13 }}>
                      Manual — choose location during refund
                    </div>
                    <div style={{ fontSize: 12, color: "#6B7280", marginTop: 2 }}>
                      Admin will select the restock location from a dropdown each time a refund is
                      processed. The fulfillment location is pre-selected but can be changed.
                    </div>
                  </div>
                </label>
              </div>
              {data.shopLocations.length > 0 && (
                <div>
                  ...
                  <select
                    aria-label="Refund location"
                    name="refundLocationId"
                    value={selectedLocId}
                  ...
                  >
                    ...
                    {data.shopLocations
                      .filter((l: ShopLocation) => l.isActive)
                      .map((loc: ShopLocation) => (
            ...
            <s-section>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 10,
                      background: portalExchangeEnabled ? "#EFF6FF" : "#F3F4F6",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      transition: "background 0.15s",
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke={portalExchangeEnabled ? "#3B82F6" : "#9CA3AF"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      style={{ transition: "stroke 0.15s" }}
                ...
                >
                  <input
                    aria-label="Enable portal exchange"
                    type="checkbox"
                    checked={portalExchangeEnabled}
                    onChange={(e) => setPortalExchangeEnabled(e.target.checked)}
                    style={{ position: "absolute", opacity: 0, width: 0, height: 0 }}
                  />
                  <span
                    style={{
                      ...
                      borderRadius: 12,
                      transition: "all 0.15s",
                      background: portalExchangeEnabled ? "#3B82F6" : "#cbd5e1",
                    }}
                  >
                    <span
                      style={{
                        position: "absolute",
                        left: portalExchangeEnabled ? 22 : 2,
                        top: 2,
                        width: 20,
                        height: 20,
                        borderRadius: 10,
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 10 }}>
                Allow customers to request an exchange (instead of a refund) directly from the
                customer portal. Customers will see a "Refund / Exchange" choice and can describe
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span
                  style={{
                    ...
                    height: 8,
                    borderRadius: 4,
                    background: portalExchangeEnabled ? "#22C55E" : "#D1D5DB",
                    transition: "background 0.15s",
                  }}
                />
                <span
                  style={{
                    fontSize: 12,
                    fontWeight: 500,
                    color: portalExchangeEnabled ? "#15803D" : "#6B7280",
                    transition: "color 0.15s",
                  }}
                >
            ...
            <s-section>
              <div style={{ fontWeight: 600, marginBottom: 4 }}>
                Allowed Fulfillment Statuses for Return Eligibility
              </div>
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 12 }}>
                Only orders with these Shopify fulfillment statuses will be eligible for returns on
                the customer portal. For Fynd-managed orders, the system also checks Fynd delivery
                  ...
                  return (
                    <label
                      ...
                      style={{
                        ...
                        borderRadius: 8,
                        border: checked ? "1.5px solid #BFDBFE" : "1px solid #E5E7EB",
                        transition: "all 0.15s",
                      }}
                    >
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked)
                            setAllowedFulfillStatuses([...allowedFulfillStatuses, status]);
                          else
                            setAllowedFulfillStatuses(
                              allowedFulfillStatuses.filter((s) => s !== status),
                            );
                        }}
              ...
              <p style={{ fontSize: 12, color: "#6d7175", marginTop: 10 }}>
                For Fynd orders, return is also allowed when Fynd status is{" "}
                <code>delivery_done</code> or <code>handed_over_to_customer</code>, regardless of
                Shopify status.
              </p>
            </s-section>
            ...
            <s-section>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 10,
                      background: fyndReturnGateEnabled ? "#EFF6FF" : "#F3F4F6",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      transition: "background 0.15s",
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke={fyndReturnGateEnabled ? "#3B82F6" : "#9CA3AF"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      style={{ transition: "stroke 0.15s" }}
                ...
                >
                  <input
                    aria-label="Enable Fynd return-status gate"
                    type="checkbox"
                    checked={fyndReturnGateEnabled}
                    onChange={(e) => {
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: 12,
                      transition: "all 0.15s",
                      background: fyndReturnGateEnabled ? "#3B82F6" : "#cbd5e1",
                    }}
                  >
                    <span
                      style={{
                        position: "absolute",
                        left: fyndReturnGateEnabled ? 22 : 2,
                        top: 2,
                        width: 20,
                        height: 20,
                        borderRadius: 10,
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 12 }}>
                When enabled, customers can only initiate a return from the portal when the Fynd
                shipment status matches one of the selected statuses below. This applies to
                Fynd-integrated orders only — non-Fynd orders use the standard fulfillment status
                gate.
              </p>
              {!fyndReturnGateEnabled && (
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 500, color: "#6B7280" }}>
                    Disabled — returns allowed based on Shopify fulfillment status only
                  </span>
                </div>
              )}
              {fyndReturnGateEnabled && (
                ...
                >
                  <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 4 }}>
                    Select allowed Fynd statuses for return initiation
                  </div>
                  <p style={{ fontSize: 12, color: "#1E40AF", marginBottom: 12 }}>
                    Customers can only create a return when the Fynd shipment status matches one of
                    the checked statuses. Select at least one status.
                  </p>
                    ...
                    return (
                      <label
                        ...
                        style={{
                          ...
                          borderRadius: 8,
                          border: checked ? "1.5px solid #93C5FD" : "1px solid #E2E8F0",
                          transition: "all 0.15s",
                        }}
                      >
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked)
                              setAllowedFyndReturnStatuses([...allowedFyndReturnStatuses, status]);
                            else
                              setAllowedFyndReturnStatuses(
                                allowedFyndReturnStatuses.filter((s) => s !== status),
                              );
                          }}
                          style={{ width: 16, height: 16, flexShrink: 0 }}
                        />
                  ...
                  <p style={{ fontSize: 11, color: "#64748B", marginBottom: 8 }}>
                    Post-delivery statuses are always allowed when the gate is enabled — customers
                    should always be able to return delivered items.
                  </p>
                  {(
                    [
                  ...
                  {allowedFyndReturnStatuses.length === 0 && (
                    ...
                    >
                      No statuses selected. When the gate is enabled, at least one Fynd status must
                      be selected — otherwise all returns for Fynd orders will be blocked.
                    </div>
                  )}
                  <p style={{ fontSize: 12, color: "#64748B", marginTop: 12, lineHeight: 1.5 }}>
                    These statuses correspond to Fynd forward-journey shipment states. When a
                    customer looks up their order on the portal, return eligibility is checked
                    against the current Fynd shipment status. Non-Fynd orders bypass this gate
                    entirely.
                  </p>
                </div>
              )}
              {fyndReturnGateEnabled && allowedFyndReturnStatuses.length > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 500, color: "#1D4ED8" }}>
                    Enabled — {allowedFyndReturnStatuses.length} status
                    {allowedFyndReturnStatuses.length !== 1 ? "es" : ""} allowed for return
                    initiation
                  </span>
                </div>
              )}
            ...
            <s-section>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 10,
                      background: fyndStatusGateEnabled ? "#FFF7ED" : "#F3F4F6",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      transition: "background 0.15s",
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke={fyndStatusGateEnabled ? "#F97316" : "#9CA3AF"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      style={{ transition: "stroke 0.15s" }}
                ...
                >
                  <input
                    aria-label="Enable Fynd refund-status gate"
                    type="checkbox"
                    checked={fyndStatusGateEnabled}
                    onChange={(e) => {
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: 12,
                      transition: "all 0.15s",
                      background: fyndStatusGateEnabled ? "#F97316" : "#cbd5e1",
                    }}
                  >
                    <span
                      style={{
                        position: "absolute",
                        left: fyndStatusGateEnabled ? 22 : 2,
                        top: 2,
                        width: 20,
                        height: 20,
                        borderRadius: 10,
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 12 }}>
                Control when refunds can be processed for Fynd-integrated orders based on the return
                bag's journey status. Non-Fynd returns are unaffected.
              </p>
              ...
              {fyndStatusGateEnabled && (
                ...
                >
                  <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 4 }}>
                    When should refunds be allowed?
                  </div>
                  <p style={{ fontSize: 12, color: "#92400E", marginBottom: 12 }}>
                    Choose a refund policy based on the return bag's journey. Refunds will be
                    blocked until the selected milestone is reached.
                  </p>
                  ...
                  >
                    {(["after_pickup", "after_delivery", "after_qc", "custom"] as const).map(
                      (preset) => {
                        ...
                        return (
                          <label
                            ...
                            style={{
                              ...
                              borderRadius: 6,
                              border: checked ? "1.5px solid #FDBA74" : "1px solid #E7E5E4",
                              transition: "all 0.15s",
                            }}
                          >
                            <input
                              ...
                              onChange={(e) => {
                                if (e.target.checked)
                                  setAllowedFyndStatuses([...allowedFyndStatuses, status]);
                                else
                                  setAllowedFyndStatuses(
                                    allowedFyndStatuses.filter((s) => s !== status),
                                  );
                              }}
                              style={{ width: 14, height: 14, flexShrink: 0 }}
                            />
                            <span
                              style={{ fontWeight: 600, fontSize: 12, fontFamily: "monospace" }}
                            >
                              {status}
                            </span>
                          </label>
            ...
            <s-section>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 10,
                      background: fyndConsolidateReturns ? "#F0FDF4" : "#F3F4F6",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      transition: "background 0.15s",
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke={fyndConsolidateReturns ? "#22C55E" : "#9CA3AF"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      style={{ transition: "stroke 0.15s" }}
                    >
                      ...
                      <path d="M5 7v4a1 1 0 001 1h4" />
                      <path d="M19 7v4a1 1 0 01-1 1h-4" />
                      <path d="M12 12v5" />
                    </svg>
                  </div>
                ...
                >
                  <input
                    aria-label="Consolidate Fynd returns"
                    type="checkbox"
                    checked={fyndConsolidateReturns}
                    onChange={(e) => setFyndConsolidateReturns(e.target.checked)}
                    style={{ position: "absolute", opacity: 0, width: 0, height: 0 }}
                  />
                  <span
                    style={{
                      ...
                      borderRadius: 12,
                      transition: "all 0.15s",
                      background: fyndConsolidateReturns ? "#3B82F6" : "#cbd5e1",
                    }}
                  >
                    <span
                      style={{
                        position: "absolute",
                        left: fyndConsolidateReturns ? 22 : 2,
                        top: 2,
                        width: 20,
                        height: 20,
                        borderRadius: 10,
              ...
              <p style={{ fontSize: 13, color: "#6d7175", marginBottom: 10 }}>
                When enabled, multiple return requests for the same Fynd order are batched into a
                single Fynd return shipment instead of creating separate pickups. This reduces
                logistics cost and courier visits.
              </p>
              {fyndConsolidateReturns && (
                <div
                  style={{
                    ...
                    {([1, 4, 8, 24] as const).map((h) => (
                      <label
                        ...
                        style={{
                          ...
                          cursor: "pointer",
                          padding: "8px 12px",
                          background: fyndConsolidateWindowHours === h ? "#DCFCE7" : "#fff",
                          borderRadius: 8,
                          border:
                            fyndConsolidateWindowHours === h
                              ? "1.5px solid #22C55E"
                              : "1px solid #E5E7EB",
                          transition: "all 0.15s",
                        }}
                      >
                        <input
                          type="radio"
                          name="fyndConsolidateWindowHours"
                          value={h}
                          checked={fyndConsolidateWindowHours === h}
                          onChange={() => setFyndConsolidateWindowHours(h)}
                        />
                        <span
                          style={{
                            fontWeight: fyndConsolidateWindowHours === h ? 600 : 400,
                            fontSize: 13,
                          }}
                        >
                          {h === 1 ? "1 hour" : `${h} hours`}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span
                  style={{
                    ...
                    height: 8,
                    borderRadius: 4,
                    background: fyndConsolidateReturns ? "#22C55E" : "#D1D5DB",
                    transition: "background 0.15s",
                  }}
                />
                <span
                  style={{
                    fontSize: 12,
                    fontWeight: 500,
                    color: fyndConsolidateReturns ? "#15803D" : "#6B7280",
                    transition: "color 0.15s",
                  }}
                >
                  {fyndConsolidateReturns
                    ? `Enabled — ${fyndConsolidateWindowHours}h batch window`
                ...
                >
                  <div>
                    ...
                    <p style={{ fontSize: 13, color: "#6d7175", margin: "4px 0 0" }}>
                      Allow gift recipients to initiate returns without the buyer's email.
                      Resolution is restricted to store credit or exchange.
                    </p>
                  </div>
                  <label
                    style={{
                  ...
                  >
                    <input
                      aria-label="Enable gift returns"
                      type="checkbox"
                      name="giftReturnsEnabled"
                      checked={giftReturnsEnabled}
                      onChange={(e) => setGiftReturnsEnabled(e.target.checked)}
                      style={{ position: "absolute", opacity: 0, width: 0, height: 0 }}
                    />
                    <span
                      style={{
                        ...
                        borderRadius: 12,
                        transition: "all 0.15s",
                        background: giftReturnsEnabled ? "#3B82F6" : "#cbd5e1",
                      }}
                    >
                      <span
                        style={{
                          position: "absolute",
                          left: giftReturnsEnabled ? 22 : 2,
                          top: 2,
                          width: 20,
                          height: 20,
                          borderRadius: 10,
              ...
              <s-section heading="Green Returns — Donate Option">
                ...
                >
                  <div>
                    ...
                    <p style={{ fontSize: 13, color: "#6d7175", margin: "4px 0 0" }}>
                      When green returns are enabled, offer customers the choice to donate the item
                  ...
                  >
                    <input
                      aria-label="Enable donate option"
                      type="checkbox"
                      name="greenReturnsDonateEnabled"
                      checked={greenReturnsDonateEnabled}
                      onChange={(e) => setGreenReturnsDonateEnabled(e.target.checked)}
                      style={{ position: "absolute", opacity: 0, width: 0, height: 0 }}
                    />
                    <span
                      style={{
                        ...
                        borderRadius: 12,
                        transition: "all 0.15s",
                        background: greenReturnsDonateEnabled ? "#3B82F6" : "#cbd5e1",
                      }}
                    >
                      <span
                        style={{
                          position: "absolute",
                          left: greenReturnsDonateEnabled ? 22 : 2,
                          top: 2,
                          width: 20,
                          height: 20,
                          borderRadius: 10,
                ...
                {greenReturnsDonateEnabled && (
                  <div style={{ marginTop: 10 }}>
                    ...
                    >
                      Donate message (shown to customer)
                    </label>
                    <input
                      aria-label="Donate message shown to customer"
                      type="text"
                      name="greenReturnsDonateMessage"
                      value={greenReturnsDonateMessage}
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        borderRadius: 12,
                        transition: "all 0.15s",
                        background: scheduledReportEnabled ? "#3B82F6" : "#cbd5e1",
                      }}
                    >
                      <span
                        style={{
                          position: "absolute",
                          left: scheduledReportEnabled ? 22 : 2,
                          top: 2,
                          width: 20,
                          height: 20,
                          borderRadius: 10,
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
                    <div>
                      ...
                      <select
                        ...
                        name="scheduledReportFrequency"
                        value={scheduledReportFrequency}
                    ...
                    {scheduledReportFrequency === "weekly" && (
                      <div>
                        ...
                        <select
                          aria-label="Scheduled report day of week"
                          name="scheduledReportDay"
                          value={scheduledReportDay}
                    ...
                    {scheduledReportFrequency === "monthly" && (
                      <div>
                        ...
                        <input
                          aria-label="Scheduled report day of month"
                          type="number"
                          name="scheduledReportDay"
                          value={scheduledReportDay}
                    ...
                    <div>
                      ...
                      <input
                        aria-label="Scheduled report recipients"
                        type="text"
                        name="scheduledReportEmails"
