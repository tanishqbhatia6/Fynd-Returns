...
const {
  ...
  createRefundMock: vi.fn<(...args: unknown[]) => Promise<unknown>>(async () => ({
    success: true, refundId: "gid://shopify/Refund/1", refundAmount: "10.00", refundCurrency: "USD", refundCreatedAt: new Date().toISOString(), refundMethod: "original",
  })),
...
function mkCtx(overrides: Partial<ReturnHandlerContext> = {}): ReturnHandlerContext {
  return {
    ...
    admin: {
      graphql: vi.fn(async () => ({
        json: async () => ({ data: { draftOrderCreate: { draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1", invoiceUrl: null, totalPrice: "0" }, userErrors: [] } } }),
      })),
    } as never,
...
describe("handleProcessRefund", () => {
  ...
  it("400 when discount_code refund method is supplied", async () => {
    const res = await handleProcessRefund(
      mkCtx(),
      { action: "process_refund", refundMethod: "discount_code" } as ReturnActionBody,
    );
    expect(res.status).toBe(400);
  });
  ...
  it("400 when storeCreditPct is out of bounds for split refund", async () => {
    const res = await handleProcessRefund(
      mkCtx(),
      { action: "process_refund", refundMethod: "both", storeCreditPct: 99 } as ReturnActionBody,
    );
    expect(res.status).toBe(400);
  });
  ...
  it("400 when split-amount mode has both amounts zero", async () => {
    const res = await handleProcessRefund(
      mkCtx(),
      { action: "process_refund", refundMethod: "both", splitMode: "amount", splitScAmount: 0, splitOrigAmount: 0 } as ReturnActionBody,
    );
    expect(res.status).toBe(400);
  });
  ...
  it("400 when Fynd status gate blocks (current status not in allowlist)", async () => {
    const ctx = mkCtx({
      returnCase: { ...mkCtx().returnCase, fyndShipmentId: "SH-1", fyndCurrentStatus: "in_transit" } as never,
      shop: {
        id: "shop-1",
        shopDomain: "store.myshopify.com",
  ...
  it("400 when createRefund returns success: false", async () => {
    ...
    const res = await handleProcessRefund(mkCtx(), { action: "process_refund" } as ReturnActionBody);
    expect(res.status).toBe(400);
  });
  ...
  it("400 when Fynd allowlist is set but currentFyndStatus is null", async () => {
    const ctx = mkCtx({
      returnCase: { ...mkCtx().returnCase, fyndShipmentId: "SH-1", fyndCurrentStatus: null } as never,
      shop: {
        id: "shop-1",
        shopDomain: "store.myshopify.com",
        settings: { allowedFyndStatusesForRefund: JSON.stringify(["return_bag_delivered"]) },
  ...
  it("falls back to fetchOrderLineItemsByName when GID resolution fails", async () => {
    const ctx = mkCtx({
      returnCase: {
        ...
        items: [{ id: "i-1", shopifyLineItemId: "3777852", qty: 1, sku: "SKU-1", price: "10.00", reasonCode: null, notes: null, title: "Item" }],
      } as never,
    });
  ...
  it("400 when neither line-item resolution strategy succeeds and createRefund fails", async () => {
    const ctx = mkCtx({
      returnCase: {
        ...mkCtx().returnCase,
        items: [{ id: "i-1", shopifyLineItemId: "3777852", qty: 1, sku: null, price: null, reasonCode: null, notes: null, title: null }],
      } as never,
    });
    fetchOrderLineItemsOnlyMock.mockResolvedValueOnce(null);
  ...
  it("writes fynd_refund_synced event on successful Fynd push", async () => {
    ...
    const eventCalls = prismaMock.returnEvent.create.mock.calls.map((c) => (c[0] as { data: { eventType: string } }).data.eventType);
  ...
  it("validates split-amount: rejects negative amounts", async () => {
    const res = await handleProcessRefund(
      mkCtx(),
      { action: "process_refund", refundMethod: "both", splitMode: "amount", splitScAmount: -1, splitOrigAmount: 5 } as ReturnActionBody,
    );
    expect(res.status).toBe(400);
  });
...
describe("handleProcessExchange", () => {
  ...
  it("400 when fetchOrder returns null", async () => {
    fetchOrderMock.mockResolvedValueOnce(null);
    const res = await handleProcessExchange(ctxWithOrder(), { action: "process_exchange" } as ReturnActionBody);
    expect(res.status).toBe(400);
  });

  it("400 when fetched order has no email", async () => {
    fetchOrderMock.mockResolvedValueOnce({ id: "gid://shopify/Order/1", email: null, lineItems: [] });
    const res = await handleProcessExchange(ctxWithOrder(), { action: "process_exchange" } as ReturnActionBody);
    expect(res.status).toBe(400);
  });
  ...
  it("happy path (no price diff): writes exchange data + redirect", async () => {
    ...
    ctx.admin = {
      graphql: vi.fn(async () => {
        ...
        return { json: async () => ({ data: { draftOrderComplete: { draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1", order: { id: "gid://shopify/Order/2", name: "#1002" } }, userErrors: [] } } }) };
      }),
    } as never;
    await expectRedirect(
  ...
  it("403 on draft-order scope error", async () => {
    fetchOrderMock.mockResolvedValueOnce({
      ...
      lineItems: [{ id: "gid://shopify/LineItem/1", title: "Item", sku: "SKU-1", price: "10.00", quantity: 1 }],
    });
    const ctx = ctxWithOrder();
    ctx.admin = {
      graphql: vi.fn(async () => ({
  ...
  it("invoice_pending flow when replacement variant is more expensive", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/V2", { id: "gid://shopify/ProductVariant/V2", price: "15.00", inventoryAvailable: 5, productTitle: "Item Bigger", variantTitle: "L", sku: "SKU-1L" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-1", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/V2", variantTitle: "L" }] }), happenedAt: new Date() },
    ] as never);
    const ctx = ctxWithOrder();
    ctx.admin = {
      graphql: vi.fn(async () => ({
        json: async () => ({ data: { draftOrderCreate: { draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1", invoiceUrl: null, totalPrice: "5.00" }, userErrors: [] } } }),
      })),
    } as never;
    await expectRedirect(
      handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody),
  ...
  it("completed_with_refund flow when replacement variant is cheaper", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/V2", { id: "gid://shopify/ProductVariant/V2", price: "7.00", inventoryAvailable: 5, productTitle: "Cheaper", variantTitle: "S", sku: "SKU-1S" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-1", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/V2" }] }), happenedAt: new Date() },
    ] as never);
    const ctx = ctxWithOrder();
    let count = 0;
    ctx.admin = {
      graphql: vi.fn(async () => {
        count++;
        if (count === 1) return { json: async () => ({ data: { draftOrderCreate: { draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1" }, userErrors: [] } } }) };
        return { json: async () => ({ data: { draftOrderComplete: { draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1", order: { id: "gid://shopify/Order/2", name: "#2" } }, userErrors: [] } } }) };
      }),
    } as never;
    await expectRedirect(
      handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody),
  ...
  it("draft order user-error (non-scope) returns 400", async () => {
    ...
    ctx.admin = {
      graphql: vi.fn(async () => ({
        json: async () => ({ data: { draftOrderCreate: { draftOrder: null, userErrors: [{ message: "Some validation error" }] } } }),
      })),
    } as never;
    const res = await handleProcessExchange(ctx, { action: "process_exchange" } as ReturnActionBody);
    expect(res.status).toBe(400);
  ...
  it("inventory blocked event written on stockout", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([
      ["gid://shopify/ProductVariant/V2", { id: "gid://shopify/ProductVariant/V2", price: "10.00", inventoryAvailable: 0, productTitle: "X", variantTitle: "L" }],
    ]));
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { id: "ev-1", payloadJson: JSON.stringify({ exchangeVariants: [{ lineItemId: "gid://shopify/LineItem/1", variantId: "gid://shopify/ProductVariant/V2" }] }), happenedAt: new Date() },
    ] as never);
...
describe("handleProcessReplacement", () => {
  ...
  it("400 when Fynd status gate blocks", async () => {
    ...
    const res = await handleProcessReplacement(ctx, { action: "process_replacement" } as ReturnActionBody);
    expect(res.status).toBe(400);
  });

  it("400 when fetchOrder returns null", async () => {
    fetchOrderMock.mockResolvedValueOnce(null);
    const res = await handleProcessReplacement(ctxWithOrder(), { action: "process_replacement" } as ReturnActionBody);
    expect(res.status).toBe(400);
  });
  ...
  it("400 when no eligible line items", async () => {
    fetchOrderMock.mockResolvedValueOnce({
      id: "gid://shopify/Order/1", email: "u@example.com", lineItems: [],
    });
    const ctx = mkCtx({
  ...
  it("409 when inventory is out of stock for picked variants", async () => {
    fetchOrderMock.mockResolvedValueOnce({
      id: "gid://shopify/Order/1", email: "u@example.com",
      lineItems: [{
        id: "gid://shopify/LineItem/1", title: "Item 1", sku: "SKU-1", price: "10.00", quantity: 1,
  ...
  it("draft order graphql user-error returns 400 (non-scope)", async () => {
    ...
    ctx.admin = {
      graphql: vi.fn(async () => ({
        json: async () => ({ data: { draftOrderCreate: { draftOrder: null, userErrors: [{ message: "Customer not eligible" }] } } }),
      })),
    } as never;
    const res = await handleProcessReplacement(ctx, { action: "process_replacement" } as ReturnActionBody);
    expect(res.status).toBe(400);
  ...
  it("falls back to fetchOrderByOrderNumber when no shopifyOrderId", async () => {
    ...
    ctx.admin = {
      graphql: vi.fn(async () => {
        count++;
        if (count === 1) return { json: async () => ({ data: { draftOrderCreate: { draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1" }, userErrors: [] } } }) };
        return { json: async () => ({ data: { draftOrderComplete: { draftOrder: { id: "gid://shopify/DraftOrder/1", name: "D1", order: { id: "gid://shopify/Order/9", name: "#9" } }, userErrors: [] } } }) };
      }),
    } as never;
    await expectRedirect(
  ...
  it("logs replacement_inventory_blocked event on stockout", async () => {
    ...
    fetchVariantInfoMock.mockResolvedValueOnce(new Map([["gid://shopify/ProductVariant/V1", { inventoryAvailable: 0 }]]));
    await handleProcessReplacement(ctxWithOrder(), { action: "process_replacement" } as ReturnActionBody);
    const eventCalls = prismaMock.returnEvent.create.mock.calls.map((c) => (c[0] as { data: { eventType: string } }).data.eventType);
