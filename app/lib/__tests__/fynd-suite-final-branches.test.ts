...
vi.mock("../observability/tracing.server", () => ({
  withSpan: async <T>(
    _n: string,
    _a: unknown,
    fn: (s: { setAttribute: () => void; setAttributes: () => void; end: () => void }) => Promise<T>,
...
describe("fynd-config — getFyndBaseUrl + getAppMode", () => {
  ...
  it("respects custom base URL when valid (with and without protocol)", () => {
    expect(getFyndBaseUrl({ fyndCustomBaseUrl: "custom.example.com" })).toBe(
...
describe("fynd.server — pure helpers", () => {
  ...
  it("isFyndPrivateUrl matches all three patterns + handles null/empty", () => {
    ...
    expect(
      isFyndPrivateUrl("https://storage.googleapis.com/fynd-things/assets/private/img.png"),
    ).toBe(true);
  ...
  it("getNormalizedCredentialsFromRaw handles empty + invalid JSON + cipher branches", () => {
    ...
    const ok = getNormalizedCredentialsFromRaw(
      JSON.stringify({ clientId: "x", clientSecret: "y", applicationToken: "tok" }),
    );
    expect(ok).toEqual({
      platform: { clientId: "x", clientSecret: "y" },
      storefront: { applicationToken: "tok" },
    });
    // ciphered valid
    const enc = "plain:" + JSON.stringify({ platform: { clientId: "p", clientSecret: "s" } });
    expect(getNormalizedCredentialsFromRaw(enc)).toEqual({
      platform: { clientId: "p", clientSecret: "s" },
    });
...
describe("fynd-payload — coverage gaps", () => {
  ...
  it.skip("parseFyndOrderDetailsForTab — return-journey AWB routing + fulfillment store object", () => {
    const payload = {
      ...
      invoice: {
        ...
        links: {
          invoice_a3: "https://a3.example",
          invoice_ewaybill: "https://ewb.example",
          label: "https://lblnew.example",
        },
      },
      orderPrice: {
        subtotal: 100,
      ...
      breakup: [
        { type: "subtotal", value: 50 },
        { type: "total", value: 45 },
        { type: "discount", value: 5 },
      ],
      tracking_details: [
        { status: "PICKED", time: "2026-01-01", message: "Picked up", tracking_details: "ignored" },
      ],
      ...
      bags: [{ articles: [{ sku: "SKU1", quantity: 2, price: 50, total: 100 }] }],
    };
  ...
  it("parseFyndOrderDetailsForTab — credit_note_id from invoice, dedupe by shipmentId", () => {
    const payload = [
      {
        shipment_id: "DUP",
        invoice: { credit_note_id: "CN-X" },
        updated_at: "2026-01-01T00:00:00Z",
      },
      {
        shipment_id: "DUP",
        invoice: { credit_note_id: "CN-Y" },
  ...
  it("extractFyndJourney — order.bags + filtered journey type + sort", () => {
        ...
        bags: [
          {
            bag_status: [
              {
                status: "bag_picked",
                bag_state_mapper: { journey_type: "forward", display_name: "Picked" },
              ...
              {
                status: "weird_status",
                bag_state_mapper: { journey_type: "return", display_name: "Weird" },
    ...
    expect(fwd.map((s) => s.status)).toEqual(["delivery_done", "bag_picked", "out_for_delivery"]);
  ...
  it("getTrackingInfoFromFyndPayload — null cases + status-as-object + dp object", () => {
    ...
    const t = getTrackingInfoFromFyndPayload(
      JSON.stringify([
        {
  ...
  it("extractCustomerFromFyndPayload + extractShippingDetailsFromFyndPayload + getPickupAddress", () => {
    ...
    const c = extractCustomerFromFyndPayload(
      JSON.stringify([
        {
          delivery_address: {
            first_name: "A",
    ...
    const s = extractShippingDetailsFromFyndPayload(
      JSON.stringify([
        {
    ...
    const a = getPickupAddressFromFyndPayload(
      JSON.stringify([
        {
          bags: [
            { return_config: { return_address: { address1: "X", city: "C", pincode: "11" } } },
          ],
        },
      ]),
    );
...
describe("fynd-webhook — pure functions", () => {
  it("classifyFyndRefundStatus — all in-progress + complete branches", () => {
    ...
    expect(classifyFyndRefundStatus("return_initiated")).toEqual({
      isInProgress: false,
      isComplete: false,
    });
    expect(classifyFyndRefundStatus("rto_initiated")).toEqual({
      isInProgress: false,
      isComplete: false,
    });
  });
  ...
  it("unwrapFyndWebhookPayload — body.data envelope + statusOrRefund chain + status object", () => {
    ...
    const r1 = unwrapFyndWebhookPayload(
      JSON.stringify({
        data: { foo: "bar" },
        shipment_id: "S1",
    ...
    const r2 = unwrapFyndWebhookPayload(
    ...
    const r3 = unwrapFyndWebhookPayload(
      JSON.stringify({ shipment_id: "S3", status: { unrecognized: { x: 1 } } }),
    );
        ...
        bags: [
          {
            ...
            bag_status_history: [
              {
                bag_state_mapper: { name: "bag_picked", journey_type: "forward" },
...
describe("fynd-returns — coverage gaps", () => {
  ...
  it("rejects manual: prefixed orders + returns 'Invalid order ID' when fyndOrderId blank", async () => {
    ...
    const r2 = await createReturnOnFynd(
      fakeClient,
      mkRC({ shopifyOrderName: "#  ", fyndOrderId: "  " }),
    );
  ...
  it("fast path detects already-exists via 'Invalid State Transition'", async () => {
    const updateShipmentStatus = vi
      .fn()
      .mockRejectedValue(new Error("Invalid State Transition: return_initiated"));
    const client = {
      getShipments: vi.fn(),
      updateShipmentStatus,
      searchShipmentsByExternalOrderId: vi.fn(),
    } as unknown as Parameters<typeof createReturnOnFynd>[0];
    const r = await createReturnOnFynd(
      client,
      mkRC({
  ...
  it("fast path failure (non-already-exists) falls through to search", async () => {
    const updateShipmentStatus = vi
      ...
      .mockResolvedValueOnce({
        statuses: [
          {
            shipments: [{ status: 200, identifier: "FYSHIP", final_state: { return_id: "RTN1" } }],
          },
        ],
      });
    const client = {
  ...
  it("getShipments 404 + targetShipId fallback uses minimal shipment + delivery_address built", async () => {
    const updateShipmentStatus = vi.fn().mockResolvedValue({
      statuses: [
        { shipments: [{ status: 200, identifier: "FYS", final_state: { shipment_id: "FYS" } }] },
      ],
    });
    const client = {
      searchShipmentsByExternalOrderId: vi
        .fn()
        .mockResolvedValue({ items: [], orderId: undefined, shipmentId: undefined }),
    ...
    const call = updateShipmentStatus.mock.calls[0][1] as {
      statuses: Array<{ shipments: Array<{ delivery_address?: unknown }> }>;
    };
      ...
      items: [
        {
          ...
          dp_details: { awb_no: "123456789012345" },
        },
      ],
    });
    expect(await refreshSingleReturn("rc-x")).toBe(true);
