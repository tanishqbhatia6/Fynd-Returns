...
describe("POST /api/integrations/gorgias-actions", () => {
  it("405 on non-POST", async () => {
    const res = await action({ request: mkReq(undefined, "GET"), params: {}, context: {} } as never);
  ...
  it("403 when Gorgias disabled", async () => {
    ...
    const res = await action({ request: mkReq({ shop: "x", api_key: "k", action: "approve", returnId: "rc-1" }), params: {}, context: {} } as never);
    expect(res.status).toBe(403);
  });

  it("403 when no API key configured", async () => {
    prismaMock.shop.findUnique.mockResolvedValueOnce({ id: "shop-1", settings: { gorgiasEnabled: true, gorgiasApiKey: null } });
    const res = await action({ request: mkReq({ shop: "x", api_key: "k", action: "approve", returnId: "rc-1" }), params: {}, context: {} } as never);
    expect(res.status).toBe(403);
  });
  ...
  it("401 on api key mismatch", async () => {
    ...
    const res = await action({ request: mkReq({ shop: "x", api_key: "wrong", action: "approve", returnId: "rc-1" }), params: {}, context: {} } as never);
  ...
  it("404 when return not found for shop", async () => {
    ...
    const res = await action({ request: mkReq({ shop: "x", api_key: "secret", action: "approve", returnId: "rc-1" }), params: {}, context: {} } as never);
  ...
  it("reject: 400 when not in initiated/pending status", async () => {
    ...
    const res = await action({ request: mkReq({ shop: "x", api_key: "secret", action: "reject", returnId: "rc-1" }), params: {}, context: {} } as never);
    expect(res.status).toBe(400);
  });
  ...
  it("reject: success uses rejectionReason body field (default used when absent)", async () => {
    ...
    const res = await action({ request: mkReq({ shop: "x", api_key: "secret", action: "reject", returnId: "rc-1", rejectionReason: "dup request" }), params: {}, context: {} } as never);
    expect(res.status).toBe(200);
    expect(prismaMock.$transaction).toHaveBeenCalled();
  });
  ...
  it("add_note: 400 when note empty", async () => {
    prismaMock.shop.findUnique.mockResolvedValueOnce(configuredShop());
    prismaMock.returnCase.findFirst.mockResolvedValueOnce({ id: "rc-1", status: "pending", adminNotes: null });
    const res = await action({ request: mkReq({ shop: "x", api_key: "secret", action: "add_note", returnId: "rc-1" }), params: {}, context: {} } as never);
    expect(res.status).toBe(400);
  });

  it("add_note: appends to existing notes", async () => {
    prismaMock.shop.findUnique.mockResolvedValueOnce(configuredShop());
    prismaMock.returnCase.findFirst.mockResolvedValueOnce({ id: "rc-1", status: "pending", adminNotes: "old note" });
    const res = await action({ request: mkReq({ shop: "x", api_key: "secret", action: "add_note", returnId: "rc-1", note: "new note" }), params: {}, context: {} } as never);
    expect(res.status).toBe(200);
    expect(prismaMock.$transaction).toHaveBeenCalled();
  });
  ...
  it("get_timeline: returns parsed events", async () => {
    ...
    prismaMock.returnEvent.findMany.mockResolvedValueOnce([
      { eventType: "status_changed", source: "admin", happenedAt: new Date("2025-01-01"), payloadJson: JSON.stringify({ from: "pending", to: "approved" }) },
      { eventType: "note_added", source: "gorgias", happenedAt: new Date("2025-01-02"), payloadJson: null },
    ]);
    const res = await action({ request: mkReq({ shop: "x", api_key: "secret", action: "get_timeline", returnId: "rc-1" }), params: {}, context: {} } as never);
    const body = await res.json();
  ...
  it("400 for unknown action type", async () => {
    prismaMock.shop.findUnique.mockResolvedValueOnce(configuredShop());
    prismaMock.returnCase.findFirst.mockResolvedValueOnce({ id: "rc-1", status: "pending" });
    const res = await action({ request: mkReq({ shop: "x", api_key: "secret", action: "nuke", returnId: "rc-1" }), params: {}, context: {} } as never);
    expect(res.status).toBe(400);
  });
