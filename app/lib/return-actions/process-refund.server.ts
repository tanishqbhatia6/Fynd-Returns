...
import { isRedirectResponse, extractErrorMessage, enrichRefundError } from "../return-action-errors.server";
  ...
  return await withSpan("return.action.process_refund", {
    ...
    try {
      ...
      if (isFyndIntegrated) {
        ...
        try {
          const raw = (shop.settings as { allowedFyndStatusesForRefund?: string | null } | null)?.allowedFyndStatusesForRefund;
          if (raw) {
      ...
      if (!isGid && !isNumericId && orderIdForRefund && !orderIdForRefund.startsWith("manual:")) {
        ...
        if (!resolved && returnCase.shopifyOrderName) {
          const order = await fetchOrderByFyndAffiliateId(admin as never, returnCase.shopifyOrderName).catch((err) => {
        ...
        if (!resolved) {
          ...
          const order = await fetchOrderByFyndAffiliateId(admin as never, cleanedOrderId).catch((err) => {
        ...
        if (!resolved && (returnCase as { fyndPayloadJson?: string | null }).fyndPayloadJson) {
          try {
            ...
            for (const raw of candidateIds) {
              ...
              seen.add(cleaned);
              const shopifyOrder = await fetchOrderByFyndAffiliateId(admin as never, cleaned).catch((err) => {
      ...
      if (lineItemsForRefund.length === 0) {
        ...
        if (orderIdForRefund) {
          minimalOrder = await fetchOrderLineItemsOnly(admin as never, orderIdForRefund).catch((err) => {
        ...
        if (!minimalOrder && returnCase.shopifyOrderName) {
          ...
          if (orderName) {
            minimalOrder = await fetchOrderLineItemsByName(admin as never, orderName).catch((err) => {
      ...
      const settings = shop.settings as { bonusCreditEnabled?: boolean; bonusCreditPct?: number; refundPaymentMethod?: string; refundStoreCreditPct?: number } | null;
      ...
      const result = await createRefund(
        admin as never, orderIdForRefund, lineItemsForRefund,
      ...
      await closeShopifyReturnBestEffort(admin as never, returnCase as never, { logEvent: logShopifyReturnEvent });
      ...
      if (returnCase.fyndShipmentId) {
        try {
          ...
          if (fyndClientResult.ok && "updateShipmentStatus" in fyndClientResult.client) {
            const fyndClient = fyndClientResult.client as import("../fynd.server").FyndPlatformClient;
