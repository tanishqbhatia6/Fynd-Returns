  ...
  try {
    ...
    if (!(returnCase as { returnRequestNo?: string | null }).returnRequestNo) {
      try {
        ...
        if (
          (idConfig.bodyMode === "sequential" || idConfig.bodyMode === "date_sequential") &&
          shop.settings?.id
        ) {
    ...
    let shopifyOrder:
      | Awaited<ReturnType<typeof fetchOrder>>
      ...
      try {
        ...
        if (!shopifyOrder) {
          ...
          if (returnCase.shopifyOrderName)
            candidates.add(returnCase.shopifyOrderName.replace(/^#/, "").trim());
          if (returnCase.shopifyOrderId && !isGid && !isNumeric)
            candidates.add(returnCase.shopifyOrderId.replace(/^#/, "").trim());
          if (fyndPayloadJson) {
          ...
          refundLogger.debug(
            ...
            "[return-detail-loader] slow path candidates",
          );
          ...
          if (candidateArray.length > 0) {
            ...
            for (let i = 0; i < results.length; i++) {
              ...
              if (r.status === "fulfilled" && r.value) {
                ...
                refundLogger.debug(
                  { candidate: candidateArray[i], resolvedId: shopifyOrder.id },
                  "[return-detail-loader] resolved via candidate",
                );
                break;
              }
            }
            if (!shopifyOrder) {
              refundLogger.warn(
                ...
                "[return-detail-loader] failed to resolve order from any candidate",
              );
            }
          }
        }
    ...
    const needsCustomerEnrich =
      !returnCase.customerName || !returnCase.customerEmailNorm || !returnCase.customerCity;
    if (needsCustomerEnrich) {
      ...
      if (shopifyOrder) {
        ...
        if (!returnCase.customerEmailNorm && shopifyOrder.email)
          enrichData.customerEmailNorm = shopifyOrder.email.toLowerCase();
        if (!(returnCase as { customerPhoneNorm?: string }).customerPhoneNorm && shopifyOrder.phone)
          enrichData.customerPhoneNorm = shopifyOrder.phone;
        if (!(returnCase as { customerCity?: string }).customerCity && addr?.city)
          enrichData.customerCity = addr.city;
        if (!(returnCase as { customerCountry?: string }).customerCountry && addr?.country)
          enrichData.customerCountry = addr.country;
        if (!(returnCase as { customerAddress1?: string }).customerAddress1 && addr?.address1)
          enrichData.customerAddress1 = addr.address1;
        if (!(returnCase as { customerAddress2?: string }).customerAddress2 && addr?.address2)
          enrichData.customerAddress2 = addr.address2;
        if (!(returnCase as { customerProvince?: string }).customerProvince && addr?.province)
          enrichData.customerProvince = addr.province;
        if (!(returnCase as { customerZip?: string }).customerZip && addr?.zip)
          enrichData.customerZip = addr.zip;
      }
      /* v8 ignore stop */
      ...
      if (fyndPayloadJson) {
        ...
        if (fyndCustomer) {
          /* v8 ignore stop */
          if (!enrichData.customerName && !returnCase.customerName && fyndCustomer.name)
            enrichData.customerName = fyndCustomer.name;
          if (!enrichData.customerEmailNorm && !returnCase.customerEmailNorm && fyndCustomer.email)
            enrichData.customerEmailNorm = fyndCustomer.email.toLowerCase();
          if (
            !enrichData.customerPhoneNorm &&
            !(returnCase as { customerPhoneNorm?: string }).customerPhoneNorm &&
            fyndCustomer.phone
          )
            enrichData.customerPhoneNorm = fyndCustomer.phone;
          if (
            !enrichData.customerCity &&
            !(returnCase as { customerCity?: string }).customerCity &&
            fyndCustomer.city
          )
            enrichData.customerCity = fyndCustomer.city;
          if (
            !enrichData.customerCountry &&
            !(returnCase as { customerCountry?: string }).customerCountry &&
            fyndCustomer.country
          )
            enrichData.customerCountry = fyndCustomer.country;
          if (
            !enrichData.customerAddress1 &&
            !(returnCase as { customerAddress1?: string }).customerAddress1 &&
            fyndCustomer.address1
          )
            enrichData.customerAddress1 = fyndCustomer.address1;
          if (
            !enrichData.customerAddress2 &&
            !(returnCase as { customerAddress2?: string }).customerAddress2 &&
            fyndCustomer.address2
          )
            enrichData.customerAddress2 = fyndCustomer.address2;
          if (
            !enrichData.customerProvince &&
            !(returnCase as { customerProvince?: string }).customerProvince &&
            fyndCustomer.province
          )
            enrichData.customerProvince = fyndCustomer.province;
          if (
            !enrichData.customerZip &&
            !(returnCase as { customerZip?: string }).customerZip &&
            fyndCustomer.zip
          )
    ...
    if (fyndPayloadJson) {
      ...
      if (
        ...
        !isLikelyFyndId(shippingInfo.trackingNumber) &&
        !(returnCase as { forwardAwb?: string }).forwardAwb
      ) {
        try {
          await prisma.returnCase.update({
            where: { id: returnCase.id },
            data: { forwardAwb: shippingInfo.trackingNumber },
          });
    ...
    if (returnCase.returnLabelJson) {
      try {
        const label = JSON.parse(returnCase.returnLabelJson) as {
          source?: string;
    ...
    const hasCompleteReturnLabel =
      ...
      (() => {
        try {
          const l = JSON.parse(returnCase.returnLabelJson!) as Record<string, unknown>;
          return !!(l.trackingUrl && l.labelUrl);
        } catch {
          return false;
        }
      })();
    const fwdShipmentPreCheck = fyndOrderDetailsTab?.shipments?.find(
    ...
    if (returnShipmentIdVal && needsFyndFetch) {
      ...
      /* v8 ignore start */
      try {
        const shopSettingsForFetch = await prisma.shopSettings.findUnique({
        ...
        if (shopSettingsForFetch) {
          ...
          if (fyndResult.ok && "searchShipmentsByExternalOrderId" in fyndResult.client) {
            ...
            if (externalOrderId) {
              ...
              const returnSearchRes = await fyndResult.client.searchShipmentsByExternalOrderId(
                externalOrderId,
                {
              ...
              const allShipments = (returnSearchRes?.items ??
                returnSearchRes?.shipments ??
              ...
              if (returnShipment) {
                const dpDetails = (returnShipment.delivery_partner_details ??
                  returnShipment.dp_details ??
                  {}) as Record<string, unknown>;
                ...
                const rCarrier =
                  String(
                    dpDetails.display_name ??
                      dpDetails.name ??
                      returnShipment.dp_name ??
                      meta.cp_name ??
                      "",
                  ).trim() || null;
                const rAwbRaw =
                  dpDetails.awb_no ?? returnShipment.awb_no ?? meta.awb_no ?? meta.awb;
                const rAwb =
                  typeof rAwbRaw === "string" && rAwbRaw.trim() && !isLikelyFyndId(rAwbRaw.trim())
                    ? rAwbRaw.trim()
                    : null;
                let rTrackingUrl =
                  String(
                    returnShipment.tracking_url ??
                      returnShipment.track_url ??
                      dpDetails.track_url ??
                ...
                const rLabelUrl =
                  (invoice ? String(invoice.label_url ?? invoiceLinks.label ?? "").trim() : "") ||
                  null;
                const rInvoiceUrl =
                  (invoice
                    ? String(invoice.invoice_url ?? invoiceLinks.invoice_a4 ?? "").trim()
                    : "") || null;
                const rStatus =
                  String(returnShipment.status ?? returnShipment.shipment_status ?? "")
                    .toLowerCase()
                    .trim() || null;
                ...
                const effCarrier =
                  ...
                  (() => {
                    try {
                      const l = JSON.parse(returnCase.returnLabelJson || "{}");
                      return l.carrier || null;
                    } catch {
                      return null;
                    }
                  })();
                ...
                try {
                  if (returnCase.returnLabelJson)
                    existingLabel = JSON.parse(returnCase.returnLabelJson);
                } catch {
                  /* ignore */
                }
                ...
                const mergedLabel = {
                  ...
                  source:
                    existingLabel.source === "fynd_webhook" ? "fynd_webhook" : "fynd_api_refresh",
                };
                ...
                returnCase = {
                  ...returnCase,
                  returnLabelJson: updateData.returnLabelJson as string,
                } as typeof returnCase;
    ...
    if (returnCase.returnLabelJson) {
      try {
        ...
        if (!rl.trackingUrl && rl.carrier && rl.trackingNumber) {
          const builtUrl = buildTrackingUrlFromCourierAndAwb(
            String(rl.carrier),
            String(rl.trackingNumber),
          );
          if (builtUrl) {
    ...
    if (
      (returnCase as { fyndSyncStatus?: string | null }).fyndSyncStatus === "processing" &&
      hasRealShipmentData
    ) {
      try {
        await prisma.returnCase.update({
          where: { id: returnCase.id },
          data: { fyndSyncStatus: "synced" },
        });
    ...
    const displayForwardAwb = isLikelyFyndId(
      (returnCase as { forwardAwb?: string | null }).forwardAwb,
    )
      ? null
      : ((returnCase as { forwardAwb?: string | null }).forwardAwb ?? null);
    const displayReturnAwb = isLikelyFyndId((returnCase as { returnAwb?: string | null }).returnAwb)
      ? null
      : ((returnCase as { returnAwb?: string | null }).returnAwb ?? null);
    /* v8 ignore stop */
    ...
    const isCodOrder =
      (shopifyOrder?.paymentGatewayNames ?? []).some((g) => COD_PATTERNS.test(g)) ||
    ...
    if (returnLabelInfo) {
      ...
      const invoiceNeedsSign = needsSign(
        rawInvoice,
        (returnLabelInfo as Record<string, unknown>).signedInvoiceAt as number | null,
      );
      ...
      if (labelNeedsSign || invoiceNeedsSign) {
        try {
          if (shopSettings) {
            const settings = {
              fyndEnvironment: (shopSettings as Record<string, unknown>).fyndEnvironment as
                | string
                | null,
              fyndCustomBaseUrl: (shopSettings as Record<string, unknown>).fyndCustomBaseUrl as
                | string
                | null,
            ...
            const [labelResult, invoiceResult] = await Promise.all([
              ...
              invoiceNeedsSign && rawInvoice
                ? signFyndUrl(settings, rawInvoice).catch(() => null)
                : null,
            ]);
            if (labelResult) {
            ...
            if (invoiceResult) {
              (returnLabelInfo as Record<string, unknown>).signedInvoiceUrl =
          ...
          /* non-fatal — show raw URL if signing fails */
        }
      }
      /* v8 ignore stop */
    }
    ...
    if (fyndOrderDetailsTab?.shipments && shopSettings) {
      ...
      for (const s of fyndOrderDetailsTab.shipments) {
        ...
        if (isFyndPrivateUrl(sAny.invoiceUrl as string | null)) {
          signPromises.push(
            signFyndUrl(fyndSignSettings, sAny.invoiceUrl as string)
              .then((r) => {
        ...
        if (isFyndPrivateUrl(sAny.labelUrl as string | null)) {
          signPromises.push(
            signFyndUrl(fyndSignSettings, sAny.labelUrl as string)
              .then((r) => {
    ...
    const defaultReturnInstructions: string | null =
    ...
    const [customerReturnHistory, customerReturnCount, isBlocklisted] = await Promise.all([
      ...
      customerEmail
        ? prisma.returnCase.count({
            where: {
              shopId: shop.id,
              customerEmailNorm: { equals: customerEmail, mode: "insensitive" },
            },
          })
        : Promise.resolve(0),
      // Blocklist check
      (async () => {
        ...
        try {
          ...
          if (returnCase.customerPhoneNorm)
            blChecks.push({ type: "phone", value: returnCase.customerPhoneNorm });
          if (blChecks.length === 0) return false;
          const blocked = await prisma.blocklistEntry.findFirst({
            where: {
              settingsId: shopSettings.id,
              OR: blChecks.map((c) => ({ type: c.type, value: c.value })),
            },
          });
          return !!blocked;
        } catch {
    ...
    const orderDateStr =
      ...
      returnCase.orderProcessedAt?.toISOString() ??
      null;
    ...
    try {
      ...
      fyndCurrentStatus =
        (returnCase as { fyndCurrentStatus?: string | null }).fyndCurrentStatus ?? null;
      ...
      if (!fyndCurrentStatus) {
        ...
        if (fyndPj) {
          ...
          fyndCurrentStatus =
            String(parsed?.status ?? parsed?.shipment_status ?? "").trim() || null;
        }
      }
    } catch {
      /* non-fatal */
    ...
    return {
      ...
      shopCurrency:
        (returnCase as { currency?: string | null }).currency ||
        shopifyOrder?.currencyCode ||
        shopSettings?.shopCurrency ||
        "USD",
      ...
      allowedFyndStatusesForRefund: (() => {
        try {
          return shopSettings?.allowedFyndStatusesForRefund
            ? (JSON.parse(shopSettings.allowedFyndStatusesForRefund) as string[])
            : [];
        } catch {
          return [];
        }
...
export default function ReturnDetail() {
  ...
  const [selectedLocationId, setSelectedLocationId] = useState<string>(
    fulfillmentLocationId ?? shopLocations[0]?.id ?? "",
  );
  ...
  const defaultRefundMethod = isCodOrder
    ...
    : ((["original", "store_credit", "both"].includes(refundPaymentMethod)
        ? refundPaymentMethod
        : "original") as "original" | "store_credit" | "both");
  /* v8 ignore stop */
  ...
  const returnRequestId =
    (returnCase as { returnRequestNo?: string | null }).returnRequestNo ??
    formatReturnRequestId(returnCase.id);
  ...
  const cancellationRequestedAt = (returnCase as { cancellationRequestedAt?: string | Date | null })
    .cancellationRequestedAt;
  const cancellationRequestedBy = (returnCase as { cancellationRequestedBy?: string | null })
    .cancellationRequestedBy;
  const cancellationReason = (returnCase as { cancellationReason?: string | null })
    .cancellationReason;
  const cancellationDeclinedAt = (returnCase as { cancellationDeclinedAt?: string | Date | null })
    .cancellationDeclinedAt;
  ...
  const forwardAwbVal =
    displayForwardAwb || (fwdShipment as { forwardAwb?: string | null })?.forwardAwb || null;
  const forwardCourier = fwdShipment ? safeStr((fwdShipment as { cpName?: string }).cpName) : "";
  const forwardTrackingUrl = fwdShipment
    ? (fwdShipment as { trackingUrl?: string | null }).trackingUrl
    : null;
  const forwardShipmentStatus = fwdShipment
    ? safeStr((fwdShipment as { shipmentStatus?: string }).shipmentStatus)
    : "";
  const forwardInvoiceNumber = fwdShipment
    ? safeStr(
        (fwdShipment as { invoiceNumber?: string }).invoiceNumber ||
          (fwdShipment as { invoiceId?: string }).invoiceId,
      )
    : "";
  const forwardInvoiceUrl = fwdShipment
    ? ((fwdShipment as { signedInvoiceUrl?: string | null }).signedInvoiceUrl ??
      (fwdShipment as { invoiceUrl?: string | null }).invoiceUrl ??
      null)
    : null;
  const forwardLabelUrl = fwdShipment
    ? ((fwdShipment as { signedLabelUrl?: string | null }).signedLabelUrl ??
      (fwdShipment as { labelUrl?: string | null }).labelUrl ??
      null)
    : null;
  ...
  const returnAwbVal =
    displayReturnAwb ||
    returnLabelInfo?.trackingNumber ||
    (retShipmentFromPayload as { returnAwb?: string | null })?.returnAwb ||
    null;
  const returnCourier =
    returnLabelInfo?.carrier ||
    (retShipmentFromPayload ? safeStr((retShipmentFromPayload as { cpName?: string }).cpName) : "");
  const returnTrackingNumber = returnLabelInfo?.trackingNumber || returnAwbVal || "";
  const returnTrackingUrl =
    returnLabelInfo?.trackingUrl ||
    (retShipmentFromPayload
      ? (retShipmentFromPayload as { trackingUrl?: string | null }).trackingUrl
      : null);
  const returnShipmentStatus =
    ((returnLabelInfo as Record<string, unknown>)?.returnStatus as string) ||
    (retShipmentFromPayload
      ? safeStr((retShipmentFromPayload as { shipmentStatus?: string }).shipmentStatus)
      : "");
  ...
  const retReturnInvoiceUrl = retShipmentFromPayload
    ? ((retShipmentFromPayload as { signedInvoiceUrl?: string | null }).signedInvoiceUrl ??
      retShipmentFromPayload.invoiceUrl ??
      null)
    : null;
  const retReturnLabelUrl = retShipmentFromPayload
    ? ((retShipmentFromPayload as { signedLabelUrl?: string | null }).signedLabelUrl ??
      (retShipmentFromPayload as { labelUrl?: string | null }).labelUrl ??
      null)
    : null;
  /* v8 ignore stop */
  ...
  const fyndCurrentStatusVal =
    (returnCase as { fyndCurrentStatus?: string | null }).fyndCurrentStatus ?? null;
  const fyndPaymentMethod = fyndOrderDetailsTab?.paymentMethod ?? null;
  const fyndSupportUrl =
    (fyndOrderDetailsTab as { supportUrl?: string | null })?.supportUrl ?? null;
      ...
      <div className="app-content layout-wide">
        ...
        {fetcher.data?.success && !fetcher.data?.error && (
          <div className="app-alert app-alert-success" style={{ marginBottom: 16 }}>
            Action completed successfully{fetcher.data.status ? ` — ${fetcher.data.status}` : ""}
          </div>
        )}
        {fetcher.data?.error && (
        ...
        {fyndError && (
          ...
          >
            ...
            <span style={{ color: "#78350f" }}>
              {(() => {
                try {
                  const d = decodeURIComponent(fyndError);
                  return d === "[object Response]" || d === "[object Object]"
                    ? "Request failed. Check Fynd configuration."
                    : d;
                } catch {
                  /* v8 ignore start */
          ...
          if (localSaysCompleted && localSaysFynd && !fyndIsAtOrPastReceived) {
            return (
              ...
              >
                ...
                <span style={{ color: "#78350f" }}>
                  Local status is <code>{returnCase.status}</code>
                  {isRefunded ? " / refunded" : ""}, but the Fynd shipment is still{" "}
                  <code>{fLocal.replace(/_/g, " ")}</code>. This usually clears once Fynd's webhook
                  for the next stage arrives. Use <em>Refresh Fynd details</em> on the right to
                  re-fetch now.
                </span>
              </div>
            );
        ...
        {fyndSuccess && (
          <div className="app-alert app-alert-success" style={{ marginBottom: 16 }}>
            {fyndSuccess === "already_synced"
              ? "Already synced to Fynd."
              : fyndSuccess === "already_exists"
                ? "Return already exists on Fynd — details loaded."
                : "Synced to Fynd successfully."}
          </div>
        )}
        {fyndRefresh && (
          <div className="app-alert app-alert-success" style={{ marginBottom: 16 }}>
            Fynd details refreshed.
          </div>
        )}
        {(fyndProcessing || fyndSyncStatus === "processing") &&
          ...
          !(
            pollCount >= MAX_POLLS ||
            Date.now() - new Date(returnCase.updatedAt).getTime() > 10 * 60 * 1000
          ) && (
            <div
              style={{
                marginBottom: 16,
            ...
            >
              ...
              <div>
                <div style={{ fontWeight: 600, color: "#1D4ED8", fontSize: 14 }}>
                  Fynd is assigning logistics
                </div>
                <div style={{ fontSize: 12, color: "#3B82F6", marginTop: 2 }}>
                  AWB and courier assignment typically take 10–30 seconds. This page will refresh
                  automatically.
                </div>
              </div>
            </div>
          ...
          fyndSyncStatus !== "pending_consolidation" && (
            <div
              style={{
                marginBottom: 16,
                padding: "14px 18px",
                background: fyndSyncStatus === "failed" ? "#FEF2F2" : "#FFF7ED",
                border: `1px solid ${fyndSyncStatus === "failed" ? "#FECACA" : "#FED7AA"}`,
                borderLeft: `4px solid ${fyndSyncStatus === "failed" ? "#DC2626" : "#F59E0B"}`,
                borderRadius: 8,
                display: "flex",
                alignItems: "center",
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <svg
                  ...
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke={fyndSyncStatus === "failed" ? "#DC2626" : "#D97706"}
                  strokeWidth="2"
                  strokeLinecap="round"
                ...
                <div>
                  <div
                    style={{
                      fontWeight: 600,
                      color: fyndSyncStatus === "failed" ? "#991B1B" : "#92400E",
                      fontSize: 14,
                    }}
                  >
                    {fyndSyncStatus === "failed"
                      ...
                      : fyndSyncStatus === "retry_scheduled"
                        ? "Fynd sync retry scheduled"
                        : "Not synced to Fynd"}
                  </div>
                  <div
                    style={{
                      fontSize: 12,
                      color: fyndSyncStatus === "failed" ? "#B91C1C" : "#B45309",
                      marginTop: 2,
                    }}
                  >
                    {fyndSyncStatus === "failed"
              ...
              <fetcher.Form
                method="post"
                action={`/api/returns/${returnCase.id}/actions`}
                style={{ flexShrink: 0 }}
              >
                <input
                  type="hidden"
                  name="json"
                  value={JSON.stringify({ action: "retry_fynd_sync" })}
                />
                <button
                  type="submit"
                  ...
                  style={{
                    ...
                    fontWeight: 600,
                    cursor: "pointer",
                    background: fyndSyncStatus === "failed" ? "#DC2626" : "#F59E0B",
                    color: "#fff",
                    border: "none",
          ...
          >
            ...
            <div>
              <div style={{ fontWeight: 600, color: "#92400E", fontSize: 14 }}>
                Queued for Fynd consolidation
              </div>
              <div style={{ fontSize: 12, color: "#B45309", marginTop: 2 }}>
                This return will be combined with other pending returns for this order and synced to
                Fynd as a single shipment. Check back after the batch window expires.
              </div>
            </div>
          </div>
        )}
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div>
                <div style={{ fontSize: 22, fontWeight: 700, color: statusConfig.color }}>
                  {statusConfig.text}
                </div>
                <div style={{ fontSize: 13, color: "#6B7280", marginTop: 2 }}>
                  Return <span style={C.mono}>{returnRequestId}</span> for order{" "}
                ...
                {daysRemaining != null && (
                  <div
                    title={
                      returnDeadline
                        ? `Return window expires ${new Intl.DateTimeFormat(shopLocale || "en", { dateStyle: "medium", timeStyle: "short", timeZone: undefined }).format(new Date(returnDeadline))}`
                        : undefined
                    }
                    style={{
                      display: "inline-flex",
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              >
                <s-button variant="secondary">
                  {isManualReturn ? "Shopify Orders" : "View in Shopify"}
                </s-button>
              </a>
            </div>
          </div>
          ...
          {unifiedState.step > 0 &&
            (() => {
              const RETURN_JOURNEY_MAP: Record<string, number> = {
                return_initiated: 1,
                bag_confirmed: 1,
                return_dp_assigned: 2,
                dp_assigned: 2,
                dp_out_for_pickup: 2,
                out_for_pickup: 2,
                return_bag_picked: 3,
                bag_picked: 3,
                return_bag_in_transit: 3,
                in_transit: 3,
                out_for_delivery_to_store: 3,
                out_for_delivery: 3,
                return_bag_out_for_delivery: 3,
                return_delivered: 4,
                delivery_done: 4,
                return_bag_delivered: 4,
                return_accepted: 4,
              ...
              const progressSteps = [
                ...
                {
                  num: 6,
                  label: isExchangeFlow ? "Exchanged" : "Refunded",
              ...
              try {
                progressSteps[0].time = returnCase.createdAt
                  ? new Date(returnCase.createdAt).toISOString()
                  : null;
              } catch {
          ...
          <div>
              ...
              {!Array.isArray(returnCase.items) || returnCase.items.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {returnCase.items.map((item) => {
                    ...
                    const shopifyItem = (shopifyOrder?.lineItems ?? []).find(
                      (li) =>
                    ...
                    const variant =
                      (item as { variantTitle?: string | null }).variantTitle ||
                      shopifyItem?.variantTitle;
                    const imageUrl =
                      (item as { imageUrl?: string | null }).imageUrl || shopifyItem?.imageUrl;
                    const rawPrice =
                      (item as { price?: string | null }).price ||
                      (shopifyItem?.discountedPrice ?? shopifyItem?.price);
                    /* v8 ignore stop */
                    const price = (() => {
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 2 }}>
                            {item.shopifyLineItemId === "manual"
                              ? item.notes || "Manual return item"
                              : title}
                          </div>
                          {variant && (
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 4 }}>
                            ...
                            {price && (
                              ...
                              >
                                {formatMoney(
                                  price,
                                  shopifyOrder?.currencyCode || shopCurrency,
                                  shopLocale,
                                )}{" "}
                                each
                              </span>
                            )}
                            {(item as { fyndSize?: string | null }).fyndSize && (
                              <span
                                style={{
                                  fontSize: 12,
                                  padding: "2px 8px",
                              ...
                              >
                                Size: {(item as { fyndSize?: string }).fyndSize}
                              </span>
                            )}
                          </div>
                          {/* Fynd item IDs — collapsible for support/troubleshooting */}
                          {((item as { fyndBagId?: string | null }).fyndBagId ||
                            (item as { fyndArticleId?: string | null }).fyndArticleId ||
                            (item as { fyndSellerIdentifier?: string | null })
                            ...
                            <details style={{ marginTop: 6 }} onClick={(e) => e.stopPropagation()}>
                              ...
                              >
                                {(item as { fyndBagId?: string | null }).fyndBagId && (
                                  ...
                                  >
                                    Bag: {(item as { fyndBagId?: string }).fyndBagId}
                                  </span>
                                )}
                                {(item as { fyndArticleId?: string | null }).fyndArticleId && (
                                  <span
                                  ...
                                  >
                                    Article: {(item as { fyndArticleId?: string }).fyndArticleId}
                                  </span>
                                )}
                                {(item as { fyndSellerIdentifier?: string | null })
                                  .fyndSellerIdentifier && (
                                  ...
                                  >
                                    SKU:{" "}
                                    {
                                      (item as { fyndSellerIdentifier?: string })
                                        .fyndSellerIdentifier
                                    }
                                  </span>
                                )}
                                {(item as { fyndItemId?: string | null }).fyndItemId && (
                                  <span
                                    style={{
                                      fontSize: 10,
                                      fontFamily: "monospace",
                                  ...
                                  >
                                    Item: {(item as { fyndItemId?: string }).fyndItemId}
                                  </span>
                                )}
                                {(item as { fyndLineNumber?: number | null }).fyndLineNumber !=
                                  null && (
                                  <span
                                    style={{
                                      fontSize: 10,
                                  ...
                                  >
                                    Line: {(item as { fyndLineNumber?: number }).fyndLineNumber}
                                  </span>
                                )}
                                {(item as { fyndPriceEffective?: string | null })
                                  .fyndPriceEffective && (
                                  <span
                                    style={{
                                      fontSize: 10,
                                  ...
                                  >
                                    Eff. Price:{" "}
                                    {(item as { fyndPriceEffective?: string }).fyndPriceEffective}
                                  </span>
                                )}
                                {(item as { fyndShipmentId?: string | null }).fyndShipmentId && (
                                  <span
                                    style={{
                                      fontSize: 10,
                                      fontFamily: "monospace",
                                  ...
                                  >
                                    Shipment: {(item as { fyndShipmentId?: string }).fyndShipmentId}
                                  </span>
                                )}
                              </div>
                            </details>
            ...
            {shopifyOrder && (
              <div style={{ ...C.card }}>
                ...
                >
                  <div>
                    <div style={C.label}>Order</div>
                    <div style={C.val}>{shopifyOrder.name || "—"}</div>
                  </div>
                  <div>
                    <div style={C.label}>Placed</div>
                    <div style={C.val}>
                      {shopifyOrder.createdAt
                        ? new Intl.DateTimeFormat(shopLocale || "en", {
                            dateStyle: "medium",
                            timeStyle: "short",
                            timeZone: undefined,
                          }).format(new Date(shopifyOrder.createdAt))
                        : "—"}
                    </div>
                  </div>
                  {shopifyOrder.email && (
                    <div>
                      ...
                      <div style={C.val}>{shopifyOrder.email}</div>
                    </div>
                  )}
                  {shopifyOrder.phone && (
                    <div>
                      <div style={C.label}>Phone</div>
                      <div style={C.val}>{shopifyOrder.phone}</div>
                    </div>
                  )}
                  {shopifyOrder.displayFulfillmentStatus && (
                    <div>
                      <div style={C.label}>Fulfillment</div>
                      <div style={C.val}>
                        {shopifyOrder.displayFulfillmentStatus.replace(/_/g, " ")}
                      </div>
                    </div>
                  )}
                  {shopifyOrder.displayFinancialStatus && (
                    <div>
                      <div style={C.label}>Payment</div>
                      <div style={C.val}>
                        {shopifyOrder.displayFinancialStatus.replace(/_/g, " ")}
                      </div>
                    </div>
                  )}
                  {(() => {
                  ...
                  {shopifyOrder.paymentGatewayNames &&
                    shopifyOrder.paymentGatewayNames.length > 0 && (
                      <div>
                ...
                {shopifyOrder.totalPrice && (
                  <div style={{ borderTop: "1px solid #F3F4F6", paddingTop: 12 }}>
                    ...
                    >
                      ...
                      {shopifyOrder.totalDiscounts &&
                        parseFloat(shopifyOrder.totalDiscounts) > 0 && (
                          <div
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                  ...
                  <div style={{ ...C.card }}>
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        ...
                        {((returnCase as { fyndOrderId?: string | null }).fyndOrderId ||
                          (returnCase.shopifyOrderName ?? "").replace(/^#/, "")) && (
                          <fetcher.Form
                            method="post"
                            action={`/api/returns/${returnCase.id}/actions`}
                          >
                            <input
                              type="hidden"
                              name="json"
                              value={JSON.stringify({ action: "refresh_fynd_details" })}
                            />
                            <s-button
                              type="submit"
                              variant="secondary"
                      ...
                      >
                        ...
                        >
                          ...
                          <strong style={{ fontSize: 14 }}>
                            {fyndSyncStatus === "failed" &&
                              `Sync failed after ${fyndSyncRetries} attempt${fyndSyncRetries !== 1 ? "s" : ""}`}
                            {fyndSyncStatus === "retry_scheduled" &&
                              `Retry #${fyndSyncRetries + 1} of 5 scheduled`}
                            {fyndSyncStatus === "pending" && "Queued for Fynd sync"}
                            {fyndSyncStatus === "processing" &&
                              "Fynd is processing \u2014 logistics assignment in progress"}
                            {fyndSyncStatus === "pending_consolidation" &&
                              "Queued for batch Fynd sync"}
                          </strong>
                        </div>
                        {/* Error details */}
                        {fyndSyncError && (
                        ...
                        {/* Retry schedule info */}
                        {fyndSyncStatus === "retry_scheduled" &&
                          (returnCase as unknown as { fyndSyncNextRetry?: Date | string | null })
                            .fyndSyncNextRetry && (
                              ...
                              {(() => {
                                try {
                                  const nextRetry = new Date(
                                    (returnCase as unknown as { fyndSyncNextRetry: Date | string })
                                      .fyndSyncNextRetry,
                                  );
                        ...
                        {fyndSyncStatus === "failed" && (
                          <div style={{ fontSize: 12, marginTop: 4, lineHeight: 1.6 }}>
                            {(() => {
                              ...
                              if (
                                /not configured|configure|settings.*integrations|client id|company id|platform api/i.test(
                                  err,
                                )
                              ) {
                                return (
                                  <span>
                                    <strong>Configuration issue</strong> \u2014 Go to{" "}
                                    <em>Settings \u2192 Integrations</em> and verify your Fynd
                                    Platform API credentials.
                                  </span>
                                );
                              }
                              if (
                                /econnrefused|enotfound|ehostunreach|network|socket hang up|dns/i.test(
                                  err,
                                )
                              ) {
                                return (
                                  <span>
                                    <strong>Network issue</strong> \u2014 Fynd API may be
                                    temporarily unreachable. Try again later.
                                  </span>
                                );
                              }
                              if (/etimedout|timeout|timed out|aborted/i.test(err)) {
                                return (
                                  <span>
                                    <strong>Timeout</strong> \u2014 The Fynd API took too long to
                                    respond. Try again or check Fynd status.
                                  </span>
                                );
                              }
                              return (
                                <span>
                                  <strong>API error</strong> \u2014 Check the Fynd dashboard for
                                  this order.
                                </span>
                              );
                            })()}
                    ...
                    {(forwardAwbVal ||
                      ...
                      >
                        ...
                        >
                          {forwardCourier && (
                            <div>
                              <div style={C.label}>Courier</div>
                              <div style={C.val}>{forwardCourier}</div>
                            </div>
                          )}
                          {forwardAwbVal && (
                            <div>
                              <div style={C.label}>AWB</div>
                              <div style={C.mono}>{forwardAwbVal}</div>
                            </div>
                          )}
                          {forwardShipmentStatus && (
                            <div>
                              <div style={C.label}>Status</div>
                              <div
                                style={{
                                  fontSize: 13,
                                  fontWeight: 600,
                                  color: forwardShipmentStatus.includes("deliver")
                                    ? "#059669"
                                    : "#D97706",
                                  textTransform: "capitalize",
                                }}
                              >
                                {forwardShipmentStatus.replace(/_/g, " ")}
                              </div>
                            </div>
                          )}
                          {forwardTrackingUrl && (
                            <div>
                              <div style={C.label}>Track</div>
                              <a
                                href={forwardTrackingUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                                style={{
                                  fontSize: 13,
                          ...
                          {forwardInvoiceNumber && (
                            <div>
                              <div style={C.label}>Invoice</div>
                              {forwardInvoiceUrl ? (
                                <a
                                  href={forwardInvoiceUrl}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  style={{
                                    fontSize: 13,
                                ...
                                >
                                  {forwardInvoiceNumber} &darr;
                                </a>
                              ) : (
                                <div style={C.mono}>{forwardInvoiceNumber}</div>
                              )}
                            </div>
                          )}
                          {forwardLabelUrl && (
                          ...
                          {fwdShipmentId && (
                            <div>
                              <div style={C.label}>Shipment ID</div>
                              <div
                                style={{ ...C.mono, fontSize: 11, wordBreak: "break-all" as const }}
                              >
                                {fwdShipmentId}
                              </div>
                            </div>
                          ...
                          {fwdFulfillmentStore && (
                            <div>
                              <div style={C.label}>Fulfillment Store</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>
                                {fwdFulfillmentStore}
                              </div>
                            </div>
                          )}
                          {fwdEstimatedDelivery && (
                            <div>
                              <div style={C.label}>Est. Delivery</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>
                                {(() => {
                                  try {
                                    return new Intl.DateTimeFormat(shopLocale || "en", {
                                      dateStyle: "medium",
                                    }).format(new Date(fwdEstimatedDelivery));
                                  } catch {
                                    /* v8 ignore start */
                                    // unreachable: Intl.DateTimeFormat does not throw on valid date strings (jsdom + node)
                                    return fwdEstimatedDelivery;
                                  }
                                  /* v8 ignore stop */
                                })()}
                              </div>
                          ...
                          {fwdWeightInfo && (
                            <div>
                              <div style={C.label}>Weight</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>{fwdWeightInfo}</div>
                            </div>
                          )}
                          {fwdDimensions && (
                            <div>
                              <div style={C.label}>Dimensions</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>{fwdDimensions}</div>
                            </div>
                          )}
                          {fwdStorePhone && (
                            <div>
                              <div style={C.label}>Store Phone</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>{fwdStorePhone}</div>
                            </div>
                          )}
                          {fwdStoreEmail && (
                            <div>
                              <div style={C.label}>Store Email</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>{fwdStoreEmail}</div>
                            </div>
                          )}
                          {fwdDpPhone && (
                            <div>
                              <div style={C.label}>DP Phone</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>{fwdDpPhone}</div>
                            </div>
                          )}
                          {fwdEwaybillUrl && (
                            <div>
                              <div style={C.label}>E-Waybill</div>
                              <a
                                href={fwdEwaybillUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                        ...
                        {fwdDeliveryAddress && (
                          ...
                          >
                            ...
                            <div style={{ fontSize: 12, color: "#374151", lineHeight: 1.5 }}>
                              {fwdDeliveryAddress.formatted ||
                                [
                                  fwdDeliveryAddress.name,
                                  fwdDeliveryAddress.address,
                                  fwdDeliveryAddress.city,
                                  fwdDeliveryAddress.state,
                                  fwdDeliveryAddress.pincode,
                                  fwdDeliveryAddress.country,
                                  fwdDeliveryAddress.phone,
                                ]
                                  .filter(Boolean)
                                  .join(", ")}
                            </div>
                        ...
                        {fwdPricing && (fwdPricing.total || fwdPricing.subtotal) && (
                          ...
                          >
                            ...
                            >
                              {fwdPricing.subtotal && (
                                <>
                                  ...
                                  <span>
                                    {formatMoney(
                                      fwdPricing.subtotal,
                                      fwdPricing.currency || shopCurrency,
                                      shopLocale,
                                    )}
                                  </span>
                                </>
                              )}
                              {fwdPricing.discount && parseFloat(fwdPricing.discount) > 0 && (
                                <>
                              ...
                              {fwdPricing.codAmount && parseFloat(fwdPricing.codAmount) > 0 && (
                                <>
                                  <span style={{ color: "#6B7280" }}>COD</span>
                                  <span>
                                    {formatMoney(
                                      fwdPricing.codAmount,
                                      fwdPricing.currency || shopCurrency,
                                      shopLocale,
                                    )}
                                  </span>
                        ...
                        {fwdTrackingDetails.length > 0 && (
                          ...
                          >
                            ...
                            >
                              Tracking History ({fwdTrackingDetails.length} events)
                            </summary>
                            <div
                              style={{
                                display: "flex",
                              ...
                              {fwdTrackingDetails.map((t, idx) => (
                                <div
                                  ...
                                  style={{
                                    display: "flex",
                                    gap: 8,
                                    paddingBottom: idx < fwdTrackingDetails.length - 1 ? 8 : 0,
                                  }}
                                >
                                  <div
                                    style={{
                                  ...
                                  >
                                    <div
                                      style={{
                                        ...
                                        height: 6,
                                        borderRadius: "50%",
                                        background: idx === 0 ? "#3B82F6" : "#D1D5DB",
                                        flexShrink: 0,
                                        marginTop: 3,
                                      }}
                                    />
                                    {idx < fwdTrackingDetails.length - 1 && (
                                      <div
                                        style={{
                                          width: 1,
                                          flex: 1,
                    ...
                    retShipmentId ? (
                      ...
                      >
                        ...
                        >
                          {(rl?.carrier || returnCourier) && (
                            <div>
                              <div style={C.label}>Courier</div>
                              <div style={C.val}>{rl?.carrier || returnCourier}</div>
                            </div>
                          )}
                          {(returnTrackingNumber || returnAwbVal) && (
                            <div>
                              <div style={C.label}>Return AWB</div>
                              <div style={C.mono}>{returnTrackingNumber || returnAwbVal}</div>
                            </div>
                          )}
                          {retStatus && (
                            <div>
                              ...
                              <div
                                style={{
                                  fontSize: 13,
                                  fontWeight: 600,
                                  color: retStatus.includes("deliver") ? "#059669" : "#D97706",
                                  textTransform: "capitalize",
                                }}
                              >
                          ...
                          {retShipmentId && (
                            <div>
                              <div style={C.label}>Return Shipment ID</div>
                              <div
                                style={{ ...C.mono, fontSize: 11, wordBreak: "break-all" as const }}
                              >
                                {retShipmentId}
                          ...
                          {retCreditNoteId && (
                            <div>
                              <div style={C.label}>Credit Note ID</div>
                              <div style={C.mono}>{retCreditNoteId}</div>
                            </div>
                          )}
                          {retFulfillmentStore && (
                            <div>
                              <div style={C.label}>Return Store</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>
                                {retFulfillmentStore}
                              </div>
                            </div>
                          )}
                          {retEstimatedDelivery && (
                            <div>
                              <div style={C.label}>Est. Return Delivery</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>
                                {(() => {
                                  try {
                                    return new Intl.DateTimeFormat(shopLocale || "en", {
                                      dateStyle: "medium",
                                    }).format(new Date(retEstimatedDelivery));
                                  } catch {
                                    /* v8 ignore start */
                                    // unreachable: Intl.DateTimeFormat does not throw on valid date strings
                                    return retEstimatedDelivery;
                                  }
                                  /* v8 ignore stop */
                                })()}
                              </div>
                          ...
                          {retWeightInfo && (
                            <div>
                              <div style={C.label}>Weight</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>{retWeightInfo}</div>
                            </div>
                          )}
                          {retDimensions && (
                            <div>
                              <div style={C.label}>Dimensions</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>{retDimensions}</div>
                            </div>
                          )}
                          {retStorePhone && (
                            <div>
                              <div style={C.label}>Store Phone</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>{retStorePhone}</div>
                            </div>
                          )}
                          {retDpPhone && (
                            <div>
                              <div style={C.label}>DP Phone</div>
                              <div style={{ fontSize: 12, color: "#374151" }}>{retDpPhone}</div>
                            </div>
                          )}
                        </div>
                        {/* v8 ignore stop */}
                        ...
                        {retPricing &&
                          (retPricing.total || retPricing.subtotal || retPricing.discount) && (
                            <details
                              style={{
                                marginTop: 10,
                                paddingTop: 10,
                                ...
                                {retPricing.subtotal && (
                                  <>
                                    ...
                                    <span>
                                      {formatMoney(
                                        retPricing.subtotal,
                                        retPricing.currency || shopCurrency,
                                        shopLocale,
                                      )}
                                    </span>
                                  </>
                                )}
                                {retPricing.discount && parseFloat(retPricing.discount) > 0 && (
                                  <>
                                    <span style={{ color: "#059669" }}>Discount</span>
                                    <span style={{ color: "#059669" }}>
                                      -
                        ...
                        {retTrackingDetails.length > 0 && (
                          ...
                          >
                            ...
                            >
                              Return Tracking History ({retTrackingDetails.length} events)
                            </summary>
                            <div
                              style={{
                                display: "flex",
                              ...
                              {retTrackingDetails.map((t, idx) => (
                                <div
                                  ...
                                  style={{
                                    display: "flex",
                                    gap: 8,
                                    paddingBottom: idx < retTrackingDetails.length - 1 ? 8 : 0,
                                  }}
                                >
                                  <div
                                    style={{
                                  ...
                                  >
                                    <div
                                      style={{
                                        ...
                                        height: 6,
                                        borderRadius: "50%",
                                        background: idx === 0 ? "#059669" : "#D1D5DB",
                                        flexShrink: 0,
                                        marginTop: 3,
                                      }}
                                    />
                                    {idx < retTrackingDetails.length - 1 && (
                                      <div
                                        style={{
                                          width: 1,
                                          flex: 1,
                      ...
                      >
                        No return shipment data yet. Click <strong>Refresh</strong> to fetch from
                        Fynd, or edit details below.
                      </div>
                    ) : null}
                          ...
                          {retJourney.map((step, idx) => (
                            <div
                              ...
                              style={{
                                display: "flex",
                                gap: 10,
                                paddingBottom: idx < retJourney.length - 1 ? 10 : 0,
                              }}
                            >
                              <div
                                style={{
                              ...
                              >
                                <div
                                  style={{
                                    ...
                                    borderRadius: "50%",
                                    background: idx === 0 ? "#059669" : "#D1D5DB",
                                    border: idx === 0 ? "2px solid #A7F3D0" : "2px solid #E5E7EB",
                                    flexShrink: 0,
                                  }}
                                />
                                {idx < retJourney.length - 1 && (
                                  <div
                                    style={{
                                      width: 2,
                                      flex: 1,
                              ...
                              <div style={{ paddingBottom: 2 }}>
                                ...
                                >
                                  {step.displayName || step.status.replace(/_/g, " ")}
                                </div>
                                {step.time && (
                    ...
                    >
                      ...
                      >
                        <div>
                          <div style={C.label}>Order ID</div>
                          <div style={{ ...C.mono, fontSize: 11, wordBreak: "break-all" as const }}>
                            {fyndOrderDetailsTab?.fyndOrderId ||
                              (returnCase as { fyndOrderId?: string | null }).fyndOrderId ||
                              (returnCase.shopifyOrderName ?? "").replace(/^#/, "") ||
                              "\u2014"}
                          </div>
                        ...
                        {fwdShipmentId && (
                          <div>
                            <div style={C.label}>Fwd Shipment ID</div>
                            <div
                              style={{ ...C.mono, fontSize: 11, wordBreak: "break-all" as const }}
                            >
                              {fwdShipmentId}
                        ...
                        {retShipmentId && (
                          <div>
                            <div style={C.label}>Ret Shipment ID</div>
                            <div
                              style={{ ...C.mono, fontSize: 11, wordBreak: "break-all" as const }}
                            >
                              {retShipmentId}
                        ...
                        {fyndReturnId && (
                          <div>
                            <div style={C.label}>Fynd Return ID</div>
                            <div
                              style={{ ...C.mono, fontSize: 11, wordBreak: "break-all" as const }}
                            >
                              {fyndReturnId}
                        ...
                        {(returnCase as { fyndReturnNo?: string | null }).fyndReturnNo && (
                          <div>
                            <div style={C.label}>Fynd Return #</div>
                            <div style={C.mono}>
                              {(returnCase as { fyndReturnNo?: string | null }).fyndReturnNo}
                            </div>
                          </div>
                        )}
                        {fyndCurrentStatusVal && (
                          <div>
                            <div style={C.label}>Current Status</div>
                            <div
                              style={{
                                fontSize: 12,
                                fontWeight: 600,
                            ...
                            >
                              {fyndCurrentStatusVal.replace(/_/g, " ")}
                            </div>
                          </div>
                        )}
                        {fyndSyncStatus && (
                          <div>
                            <div style={C.label}>Sync</div>
                            <div
                              style={{
                                fontSize: 12,
                                fontWeight: 600,
                        ...
                        {fyndPaymentMethod && (
                          <div>
                            ...
                            >
                              {fyndPaymentMethod.replace(/_/g, " ")}
                            </div>
                          </div>
                        )}
                        {fyndSupportUrl && (
                          <div>
                            <div style={C.label}>Support</div>
                            <a
                              href={fyndSupportUrl}
                              target="_blank"
                              rel="noopener noreferrer"
                          ...
                          >
                            ...
                            >
                              <div className="app-field">
                                <label style={{ fontSize: 12, fontWeight: 600 }}>Carrier</label>
                                <input
                                  type="text"
                                  name="carrier"
                                  defaultValue={rl?.carrier ?? ""}
                                  ...
                                  onChange={(e) => {
                                    const h = e.target
                                      .closest("form")
                                      ?.querySelector('input[name="json"]') as HTMLInputElement;
                                    if (h) {
                                      const v = JSON.parse(h.value);
                                      v.carrier = e.target.value;
                                      h.value = JSON.stringify(v);
                                    }
                                  }}
                                />
                              </div>
                              <div className="app-field">
                                <label style={{ fontSize: 12, fontWeight: 600 }}>Tracking #</label>
                                <input
                                  type="text"
                                  name="trackingNumber"
                                  defaultValue={rl?.trackingNumber ?? ""}
                                  placeholder="AWB"
                                  className="app-input"
                                  style={{ fontSize: 13 }}
                                  onChange={(e) => {
                                    ...
                                    if (h) {
                                      const v = JSON.parse(h.value);
                                      v.trackingNumber = e.target.value;
                                      h.value = JSON.stringify(v);
                                    }
                                  }}
                                />
                          ...
                          >
                            <input
                              ...
                              value={JSON.stringify({
                                ...
                                returnInstructions: defaultReturnInstructions ?? "",
                              })}
                            />
                            <div className="app-field" style={{ marginBottom: 8 }}>
                              ...
                              <textarea
                                ...
                                defaultValue={defaultReturnInstructions ?? ""}
                                rows={2}
                                placeholder="e.g. Pack items securely and drop off at..."
                                style={{
                                  width: "100%",
                                  padding: 8,
                                  borderRadius: 6,
                                ...
                                onChange={(e) => {
                                  ...
                                  if (h) {
                                    const v = JSON.parse(h.value);
                                    v.returnInstructions = e.target.value;
                                    h.value = JSON.stringify(v);
                                  }
                                }}
                              />
                      ...
                      <div style={{ paddingTop: 8, borderTop: "1px solid #F3F4F6" }}>
                        ...
                        {showRawFynd && (
                          <div style={{ marginTop: 8, minWidth: 0 }}>
                            <PayloadViewer
                              rawPayload={fyndPayloadInfo?.rawJson ?? null}
                              title="Fynd Payload"
                            />
                          </div>
                        )}
            ...
            <div style={{ ...C.card }}>
              ...
              {!Array.isArray(returnCase.events) || returnCase.events.length === 0 ? (
                <div style={{ padding: 20, textAlign: "center", color: "#9CA3AF", fontSize: 14 }}>
                  No events yet. Activity will appear here as the return progresses.
                </div>
              ) : (
                <div style={{ position: "relative", paddingLeft: 28 }}>
                  ...
                  {returnCase.events.map((ev, i) => {
                    ...
                    const sourceColor =
                      ev.source === "fynd_webhook"
                        ? "#059669"
                        : ev.source === "portal"
                          ? "#2563EB"
                          : ev.source === "system"
                            ? "#8B5CF6"
                            : ev.source === "shopify_webhook"
                              ? "#0EA5E9"
                              : "#64748B";
                    const sourceLabel =
                      ev.source === "fynd_webhook"
                    ...
                    try {
                      if (ev.payloadJson)
                        evPayload = JSON.parse(ev.payloadJson) as Record<string, unknown>;
                    } catch {
                      evPayload = null;
                    }
                    const evAdminEmail = evPayload?.adminEmail as string | null | undefined;
                    const isFyndSyncEvent = [
                      "fynd_sync",
                      "fynd_sync_failed",
                      "fynd_sync_retry_success",
                      "fynd_sync_retries_exhausted",
                    ...
                    return (
                      <div
                        ...
                        style={{
                          position: "relative",
                          paddingBottom: i < returnCase.events.length - 1 ? 20 : 0,
                        }}
                      >
                        <div
                          style={{
                            ...
                            height: 12,
                            borderRadius: "50%",
                            background: isLatest ? sourceColor : "#D1D5DB",
                            border: "2px solid #fff",
                            boxShadow: isLatest ? `0 0 0 3px ${sourceColor}30` : "none",
                          }}
                        />
                        <div>
                          ...
                          {isFyndSyncEvent && evPayload && (
                            <div
                              style={{
                                ...
                                lineHeight: 1.6,
                                background:
                                  evPayload.status === "success"
                                    ? "#F0FDF4"
                                    : evPayload.status === "failed"
                                      ? "#FEF2F2"
                                      : "#F8FAFC",
                            ...
                            >
                              ...
                              >
                                ...
                                {!!evPayload.action && (
                                  <span style={{ fontSize: 11, color: "#6B7280" }}>
                                    {String(evPayload.action).replace(/_/g, " ")}
                                  </span>
                                )}
                                {typeof evPayload.durationMs === "number" && (
                                  <span style={{ fontSize: 11, color: "#9CA3AF" }}>
                                    {String(evPayload.durationMs)}ms
                                  </span>
                                )}
                                {typeof evPayload.attempt === "number" && (
                                  <span style={{ fontSize: 11, color: "#9CA3AF" }}>
                                    attempt #{String(evPayload.attempt)}
                                  </span>
                                )}
                                {typeof evPayload.retryAttempt === "number" && (
                                  <span style={{ fontSize: 11, color: "#9CA3AF" }}>
                                    retry #{String(evPayload.retryAttempt)}
                                  </span>
                                )}
                              </div>
                              {/* Success: show IDs */}
                              {evPayload.status === "success" &&
                                !!(evPayload.fyndReturnId || evPayload.fyndShipmentId) && (
                                  <div
                                    style={{
                                      display: "flex",
                                      gap: 12,
                                  ...
                                  >
                                    {!!evPayload.fyndReturnId && (
                                      <span>
                                        <strong>Return ID:</strong>{" "}
                                        <code style={{ fontFamily: "monospace", fontSize: 10 }}>
                                          {String(evPayload.fyndReturnId)}
                                        </code>
                                      </span>
                                    )}
                                    {!!evPayload.fyndShipmentId && (
                                      <span>
                                        <strong>Shipment:</strong>{" "}
                                        <code style={{ fontFamily: "monospace", fontSize: 10 }}>
                                          {String(evPayload.fyndShipmentId)}
                                        </code>
                                      </span>
                                    )}
                                    {!!evPayload.fyndOrderId && (
                                      <span>
                                        <strong>Order:</strong>{" "}
                                        <code style={{ fontFamily: "monospace", fontSize: 10 }}>
                                          {String(evPayload.fyndOrderId)}
                                        </code>
                                      </span>
                                    )}
                                  </div>
                              ...
                              {evPayload.status === "failed" && !!evPayload.error && (
                                <div style={{ fontSize: 11 }}>
                                  ...
                                  <span style={{ color: "#991B1B" }}>
                                    {String(evPayload.error).slice(0, 300)}
                                  </span>
                                  {!!evPayload.retryScheduled && !!evPayload.nextRetryAt && (
                                    <div style={{ marginTop: 4, color: "#B45309" }}>
                                      Retry scheduled: {String(evPayload.nextRetryAt)}
                                    </div>
                                  )}
                                </div>
                              )}
                              ...
                              {ev.eventType === "fynd_sync_retries_exhausted" && (
                                <div style={{ fontSize: 11, color: "#991B1B" }}>
                                  <strong>
                                    All {String(evPayload.maxRetries ?? evPayload.attempts)} retries
                                    exhausted.
                                  </strong>
                                  {!!evPayload.lastError && (
                                    <span>
                                      {" "}
                                      Last error: {String(evPayload.lastError).slice(0, 200)}
                                    </span>
                                  )}
                                  <div style={{ marginTop: 4, color: "#92400E" }}>
                                    Use the &ldquo;Sync to Fynd&rdquo; button above, or process the
                                    refund manually.
                                  </div>
                                </div>
                              )}
          ...
          <div>
            ...
            {hasCancellationRequest && (
              ...
              >
                <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div
                      style={{ fontWeight: 700, fontSize: 14, color: "#92400E", marginBottom: 4 }}
                    >
                    ...
                    <div style={{ fontSize: 12, color: "#78350F", marginBottom: 2 }}>
                      Requested on{" "}
                      {cancellationRequestedAt
                        ? new Date(cancellationRequestedAt as string).toLocaleString()
                        : "—"}
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                      ...
                      <fetcher.Form method="post" action={`/api/returns/${returnCase.id}/actions`}>
                        <input
                          type="hidden"
                          name="json"
                          value={JSON.stringify({ action: "decline_cancellation" })}
                        />
                        <button
                          type="submit"
                          disabled={fetcher.state !== "idle"}
                ...
                >
                  ...
                  <div className="app-modal-body">
                    <p style={{ margin: 0, fontSize: 14 }}>
                      Are you sure you want to approve the customer&apos;s cancellation request for
                      order <strong>{returnCase.shopifyOrderName || "—"}</strong>?
                    </p>
                    <p style={{ margin: "10px 0 0", fontSize: 13, color: "#6B7280" }}>
                      This will cancel the return and cannot be undone. If synced to Fynd, a
                      cancellation request will also be sent to Fynd.
                    </p>
                  </div>
                  <div className="app-modal-actions">
                    ...
                    <fetcher.Form
                      method="post"
                      action={`/api/returns/${returnCase.id}/actions`}
                      onSubmit={() => setShowApproveCancelModal(false)}
                    >
                      <input
                        type="hidden"
                        name="json"
                        value={JSON.stringify({ action: "approve_cancellation" })}
                      />
                      <button
                        type="submit"
                        disabled={fetcher.state !== "idle"}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <>
                    ...
                    {(() => {
                      const isExchangeFlow =
                        (returnCase.resolutionType || "").toLowerCase() === "exchange";
                                ...
                                {(
                                  [
                                    {
                                      value: "refund",
                                      label: "Refund",
                                      desc: "Refund to customer's payment method",
                                      color: "#2563EB",
                                      bg: "#DBEAFE",
                                      border: "#93C5FD",
                                    },
                                    {
                                      value: "exchange",
                                      label: "Exchange",
                                      desc: "Create a new order with replacement items",
                                      color: "#059669",
                                      bg: "#DCFCE7",
                                      border: "#BBF7D0",
                                    },
                                    {
                                      value: "store_credit",
                                      label: "Store Credit",
                                      desc: "Issue store credit to customer's account",
                                      color: "#7C3AED",
                                      bg: "#F3E8FF",
                                      border: "#D8B4FE",
                                    },
                                    {
                                      value: "replacement",
                                      label: "Replacement",
                                      desc: "Send the same item(s) again",
                                      color: "#EA580C",
                                      bg: "#FFF7ED",
                                      border: "#FED7AA",
                                    },
                                  ...
                                  <label
                                    ...
                                    style={{
                                      ...
                                      fontSize: 13,
                                      background:
                                        selectedResolutionType === opt.value
                                          ? opt.bg
                                          : "transparent",
                                      border:
                                        selectedResolutionType === opt.value
                                  ...
                                  >
                                    <input
                                      type="radio"
                                      checked={selectedResolutionType === opt.value}
                                      onChange={() => setSelectedResolutionType(opt.value)}
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                          ...
                          >
                            ...
                            <s-button
                              type="submit"
                              variant="secondary"
                              disabled={fetcher.state !== "idle" || !rejectReason.trim()}
                              style={{ width: "100%" }}
                            >
                              Confirm
                            </s-button>
                ...
                {(isPostApproval || isCompleted) &&
                  ...
                  (() => {
                    ...
                    const refundGatedByFynd =
                      isFyndIntegrated &&
                      refundGateStatuses.length > 0 &&
                      (!currentFyndStatusLower ||
                        !refundGateStatuses.includes(currentFyndStatusLower));
                    const gatePresetLabel =
                      refundGatePreset && PRESET_LABELS[refundGatePreset as RefundGatePreset]
                    ...
                    return (
                      <>
                          ...
                          >
                            ...
                            {gatePresetLabel && gatePresetLabel !== "Custom" && (
                              <div style={{ marginTop: 2 }}>
                                Refund available: <strong>{gatePresetLabel}</strong>
                              </div>
                            )}
                          </div>
                        )}
                        <s-button
                          type="button"
                          variant="primary"
                          disabled={fetcher.state !== "idle" || refundGatedByFynd}
                        ...
                        {showRefundConfirm && (
                          <div
                            ...
                            onClick={() => {
                              if (!fetcher.data?.error) setShowRefundConfirm(false);
                            }}
                          >
                            <div
                              className="app-modal"
                              ...
                              >
                                ...
                                >
                                  ...
                                  {isCodOrder && (
                                    ...
                                    >
                                      ...
                                      <span>
                                        <strong>COD order</strong> — Refund to original payment is
                                        not available. Use Store credit.
                                      </span>
                                    </div>
                                  )}
                                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                                    {[
                                      ...
                                      },
                                      {
                                        ...
                                        label: "Store credit",
                                        desc: "Issue as store credit to customer's account",
                                        color: "#22C55E",
                                        bg: "#F0FDF4",
                                        border: "#22C55E",
                                        disabled: false,
                                      },
                                      {
                                        ...
                                        desc: isCodOrder
                                          ? "Not available for COD orders"
                                          : "Split between original payment and store credit",
                                        color: "#F59E0B",
                                        bg: "#FFFBEB",
                                        border: "#F59E0B",
                                        disabled: isCodOrder,
                                      ...
                                      >
                                        <input
                                          type="radio"
                                          checked={modalRefundMethod === opt.value}
                                          disabled={opt.disabled}
                                          onChange={() =>
                                            !opt.disabled && setModalRefundMethod(opt.value)
                                          }
                                          style={{ accentColor: opt.color }}
                                        />
                                        <div style={{ flex: 1 }}>
                                  ...
                                  {modalRefundMethod === "both" && (
                                    ...
                                    >
                                      ...
                                      >
                                        ...
                                        <strong>
                                          {formatMoney(
                                            String(refundItemTotal.toFixed(2)),
                                            shopCurrency,
                                            shopLocale,
                                          )}
                                        </strong>
                                      ...
                                      <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            ...
                                            const sc =
                                              Math.round(
                                                refundItemTotal * (modalStoreCreditPct / 100) * 100,
                                              ) / 100;
                                            const orig =
                                              Math.round((refundItemTotal - sc) * 100) / 100;
                                            setSplitScAmount(sc.toFixed(2));
                                            setSplitOrigAmount(
                                              orig >= 0 ? orig.toFixed(2) : "0.00",
                                            );
                                          }}
                                          style={{
                                            padding: "4px 12px",
                                            ...
                                            border: "1px solid #D97706",
                                            background:
                                              splitMode === "amount" ? "#F59E0B" : "transparent",
                                      ...
                                      {splitMode === "percentage" ? (
                                        <>
                                          ...
                                          >
                                            ...
                                            >
                                              Store credit: {modalStoreCreditPct}%
                                            </span>
                                            ...
                                            >
                                              Original: {100 - modalStoreCreditPct}%
                                            </span>
                                          </div>
                                          <input
                                            ...
                                            onChange={(e) =>
                                              setModalStoreCreditPct(parseInt(e.target.value, 10))
                                            }
                                            ...
                                            <div style={{ flex: 1 }}>
                                              ...
                                              <input
                                                ...
                                                onChange={(e) => {
                                                  ...
                                                  const remaining =
                                                    Math.round((refundItemTotal - orig) * 100) /
                                                    100;
                                                  setSplitScAmount(
                                                    remaining >= 0 ? remaining.toFixed(2) : "0.00",
                                                  );
                                            ...
                                            if (diff > 0.01) {
                                              return (
                                                ...
                                                >
                                                  ...
                                                  ) does not match eligible amount (
                                                  {formatMoney(
                                                    String(refundItemTotal.toFixed(2)),
                                                    shopCurrency,
                                                    shopLocale,
                                  ...
                                  {modalRefundMethod === "store_credit" && (
                                    ...
                                    >
                                      Requires new customer accounts in Shopify. Order must have an
                                      associated customer.
                                    </div>
                                  )}
                                </div>
                                ...
                                {bonusCreditEnabled &&
                                  ...
                                  (() => {
                                    ...
                                    const bonusAmt =
                                      Math.round(refundItemTotal * (bonusCreditPct / 100) * 100) /
                                      100;
                                    /* v8 ignore start */
                                    ...
                                    /* v8 ignore stop */
                                    const totalCredit =
                                      Math.round((scPortion + bonusAmt) * 100) / 100;
                                    return (
                                      <div
                                        style={{
                                          padding: 14,
                                    ...
                                    value={JSON.stringify({
                                      ...
                                      storeCreditPct:
                                        modalRefundMethod === "both"
                                          ...
                                          : undefined,
                                      splitMode:
                                        modalRefundMethod === "both" ? splitMode : undefined,
                                      splitScAmount:
                                        modalRefundMethod === "both" && splitMode === "amount"
                                          ? parseFloat(splitScAmount) || 0
                                          : undefined,
                                      splitOrigAmount:
                                        modalRefundMethod === "both" && splitMode === "amount"
                                      ...
                                      (modalRefundMethod === "store_credit" ||
                                        ...
                                        ? {
                                            bonusAmount:
                                              refundItemTotal > 0
                                                ? Math.round(
                                                    refundItemTotal * (bonusCreditPct / 100) * 100,
                                                  ) / 100
                                                : undefined,
                                          }
                                        : {}),
                ...
                {(isPostApproval || isCompleted) &&
                  ...
                  !exchangeResolved && (
                    ...
                    >
                      Manual return — process refund in Shopify Admin for{" "}
                      <strong>{returnCase.shopifyOrderName || "--"}</strong>
                    </div>
                  )}
                {(isPostApproval || isCompleted) &&
                  returnCase.resolutionType === "replacement" &&
                  !returnCase.exchangeOrderId &&
                  !isManualReturn && (
                    <>
                      <s-button
                      ...
                      {replacementBlockedByFynd && (
                        ...
                        >
                          <strong>Replacement unavailable</strong> — Return bag not yet received at
                          warehouse. Current Fynd status:{" "}
                          <code
                            style={{ background: "#FDE68A", padding: "1px 5px", borderRadius: 3 }}
                          >
                            {fyndCurrentStatus}
                          </code>
                          . Available after{" "}
                      ...
                      {showReplacementConfirm && (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              <p style={{ margin: 0 }}>
                                Create a new Shopify order to reship the same item(s) at no charge
                                for order <strong>{returnCase.shopifyOrderName || "--"}</strong>.
                              </p>
                              <div
                                style={{
                                  padding: 12,
                              ...
                              >
                                A real Shopify order will be created with a 100% applied discount
                                (no charge to the customer). It will appear under{" "}
                                <strong>Orders</strong> in Shopify Admin so the warehouse can
                                fulfill it.
                              </div>
                              {fetcher.data?.error && (
                                <div
                            ...
                            <div className="app-modal-actions">
                              ...
                              >
                                <input
                                  type="hidden"
                                  name="json"
                                  value={JSON.stringify({ action: "process_replacement" })}
                                />
                                <s-button
                                  type="submit"
                                  variant="primary"
                ...
                {(isPostApproval || isCompleted) &&
                  returnCase.resolutionType === "exchange" &&
                  !returnCase.exchangeOrderId &&
                  !isManualReturn && (
                    <>
                      <s-button
                      ...
                      {exchangeBlockedByFynd && (
                        ...
                        >
                          <strong>Exchange unavailable</strong> — Return bag not yet received at
                          warehouse. Current Fynd status:{" "}
                          <code
                            style={{ background: "#FDE68A", padding: "1px 5px", borderRadius: 3 }}
                          >
                      ...
                      {showExchangeConfirm && (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              <p style={{ margin: 0 }}>
                                Create a draft order in Shopify with the same items from order{" "}
                                <strong>{returnCase.shopifyOrderName || "--"}</strong>.
                              </p>
                              {(returnCase as { exchangePreference?: string | null })
                                .exchangePreference && (
                                <div
                                  style={{
                                    padding: 12,
                                ...
                                >
                                  <div
                                    style={{ fontWeight: 600, color: "#92400E", marginBottom: 4 }}
                                  >
                                    Customer exchange preference:
                                  </div>
                              ...
                              >
                                A draft order will be created with the customer's email and the
                                return items. You can then complete the order in Shopify Admin.
                              </div>
                              {fetcher.data?.error && (
                                <div
                                  style={{
                            ...
                            <div className="app-modal-actions">
                              ...
                              >
                                <input
                                  type="hidden"
                                  name="json"
                                  value={JSON.stringify({ action: "process_exchange" })}
                                />
                                <s-button
                                  type="submit"
                                  variant="primary"
                ...
                {returnCase.exchangeOrderId &&
                  (() => {
                    ...
                    const isDraftGid = returnCase.exchangeOrderId.startsWith(
                    ...
                    try {
                      if (returnCase.exchangeItemsJson)
                        exchangeItems = JSON.parse(returnCase.exchangeItemsJson);
                    } catch {
                      /* v8 ignore start */
                    ...
                    for (let i = events.length - 1; i >= 0; i--) {
                      ...
                      if (
                        !ev ||
                        (ev.eventType !== "exchange_created" &&
                          ev.eventType !== "replacement_created")
                      )
                        continue;
                    ...
                    const priceDiff =
                      typeof exchangePayload.priceDiff === "number"
                        ? exchangePayload.priceDiff
                        : null;
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <span style={{ fontWeight: 700, fontSize: 14, color: panelText }}>
                            {headlineLabel}
                          </span>
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                          ...
                          {priceDiff != null && Math.abs(priceDiff) > 0.001 && (
                            ...
                            >
                              <span style={{ color: panelText }}>
                                {priceDiff > 0 ? "Customer owes" : "Refunded to customer"}
                              </span>
                              <span style={{ fontWeight: 700, color: panelText }}>
                          ...
                          {flow === "completed_with_refund" &&
                            exchangePayload.refund?.success &&
                            exchangePayload.refund?.amount && (
                              <div style={{ fontSize: 12, color: "#15803D" }}>
                                ✓ Difference refunded ({exchangePayload.refund.amount} {currency})
                              </div>
                            )}
                          {flow === "completed_with_refund" &&
                            exchangePayload.refund?.success === false && (
                              <div
                                style={{
                                  fontSize: 12,
                                  color: "#B91C1C",
                          ...
                          <a
                            ...
                            style={{
                              fontSize: 12,
                              fontWeight: 600,
                              color: flow === "invoice_pending" ? "#B45309" : "#059669",
                              marginTop: 4,
                            }}
                          >
                ...
                {isOrderCancellable && (
                  <>
                    <s-button
                      ...
                      style={{ width: "100%", borderColor: "#FECACA", color: "#DC2626" }}
                    >
                      Cancel Order
                    </s-button>
                    {showCancelOrder && (
                      <div className="app-modal-overlay" onClick={() => setShowCancelOrder(false)}>
                        ...
                        >
                          ...
                          >
                            <p style={{ margin: 0 }}>
                              Cancel the Shopify order for{" "}
                              <strong>{returnCase.shopifyOrderName || "--"}</strong>. This will
                              cancel the order directly in Shopify.
                            </p>
                            <div
                              style={{
                                padding: 14,
                            ...
                            <p
                              style={{ color: "#DC2626", fontWeight: 500, fontSize: 13, margin: 0 }}
                            >
                              This action cannot be undone.
                            </p>
                          </div>
                ...
                {isRefunded &&
                  (() => {
                    ...
                    const storedBonusAmount = (returnCase as { bonusCreditAmount?: string | null })
                      .bonusCreditAmount;
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                            {refundInfo.amount && (
                              ...
                              >
                                ...
                                <span style={{ fontWeight: 700, color: "#166534" }}>
                                  {formatMoney(
                                    refundInfo.amount,
                                    refundInfo.currency || shopCurrency,
                                    shopLocale,
                                  )}
                                </span>
                              </div>
                            ...
                            {displayBonus && parseFloat(displayBonus) > 0 && (
                              ...
                              >
                                ...
                                <span style={{ fontWeight: 600 }}>
                                  +{formatMoney(displayBonus, shopCurrency, shopLocale)}
                                </span>
                              </div>
                            )}
                              ...
                              >
                                ...
                                <span style={{ color: "#166534" }}>
                                  {new Intl.DateTimeFormat(shopLocale || "en", {
                                    ...
                                    timeStyle: "short",
                                    timeZone: undefined,
                                  }).format(new Date(refundInfo.createdAt))}
                                </span>
                              </div>
                            )}
                            {refundInfo.source && (
                              ...
                              >
                                ...
                                <span style={{ color: "#166534" }}>
                                  {refundInfo.source === "admin"
                                    ? "Admin"
                                    : refundInfo.source === "fynd_webhook"
                                      ? "Fynd"
                                      : refundInfo.source === "auto_fynd_credit_note"
                                        ? "Auto (Credit Note)"
                                        : refundInfo.source}
                                </span>
                                {/* v8 ignore stop */}
            ...
            <div style={{ ...C.card }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  <div style={C.label}>Return ID</div>
                  <div style={C.mono}>{returnRequestId}</div>
                </div>
                <div>
                  <div style={C.label}>Order</div>
                  <div style={C.val}>{returnCase.shopifyOrderName || "—"}</div>
                </div>
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2 }}>
                    <span
                      style={{
                        ...
                        background: unifiedState.bg,
                        color: unifiedState.color,
                        border: `1px solid ${unifiedState.border}`,
                      }}
                    >
                      <span
                        style={{
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
                    <div style={C.label}>App Status</div>
                    <div style={{ ...C.val, fontSize: 12 }}>{returnCase.status}</div>
                  </div>
                  <div>
                ...
                <div>
                  <div style={C.label}>Created</div>
                  <div style={{ ...C.val, fontSize: 13 }}>
                    {new Intl.DateTimeFormat(shopLocale || "en", {
                      dateStyle: "medium",
                      timeStyle: "short",
                      timeZone: undefined,
                    }).format(new Date(returnCase.createdAt))}
                  </div>
                </div>
                <div>
                  <div style={C.label}>Last Updated</div>
                  <div style={{ ...C.val, fontSize: 13 }}>
                    {new Intl.DateTimeFormat(shopLocale || "en", {
                      dateStyle: "medium",
                      timeStyle: "short",
                      timeZone: undefined,
                    }).format(new Date(returnCase.updatedAt))}
                  </div>
                </div>
                {(displayForwardAwb || displayReturnAwb) && (
                  <>
                    {displayForwardAwb && (
                      <div>
                        <div style={C.label}>Forward AWB</div>
                        <div style={C.mono}>{displayForwardAwb}</div>
                      </div>
                    )}
                    {displayReturnAwb && (
                      <div>
                        <div style={C.label}>Return AWB</div>
                        <div style={C.mono}>{displayReturnAwb}</div>
                      </div>
                    )}
                  </>
                )}
            ...
            {customerReturnCount > 0 && (
              ...
              >
                ...
                <div
                  style={{
                    fontSize: 14,
                    color: customerReturnCount >= 3 ? "#991B1B" : "#374151",
                    marginBottom: 8,
                  }}
                >
                  <strong>{customerReturnCount}</strong>{" "}
                  {customerReturnCount === 1 ? "return" : "returns"} from this customer
                </div>
                {customerEmail && (
                  <Link
                    to={`/app/customers?q=${encodeURIComponent(customerEmail)}`}
                    style={{
                      fontSize: 13,
                      fontWeight: 600,
                      color: "#2563EB",
                ...
                {customerReturnHistory.length > 0 && (
                  <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                    {customerReturnHistory.slice(0, 5).map((prev) => (
                      ...
                      >
                        <span style={{ fontWeight: 600 }}>
                          {prev.returnRequestNo || prev.id.slice(-8).toUpperCase()}
                        </span>
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                          <span style={{ fontSize: 11, color: "#6b7280" }}>
                            {new Intl.DateTimeFormat(shopLocale || "en", {
                              ...
                              hour: "2-digit",
                              minute: "2-digit",
                            }).format(new Date(prev.createdAt))}
                          </span>
                          <span
                            style={{
                              fontSize: 10,
                              ...
                              padding: "2px 6px",
                              borderRadius: 4,
                              background: getStatusBg(prev.status),
                              color: getStatusColor(prev.status),
                              textTransform: "uppercase",
                            }}
                          >
                            {prev.status}
            ...
            {(() => {
              ...
              const cPhone =
                (shopifyOrder as { phone?: string | null } | null)?.phone ||
                returnCase.customerPhoneNorm;
              const cName =
              ...
              return (
                <div style={{ ...C.card }}>
                    ...
                    {customerReturnCount > 1 && (
                      <span
                        style={{
                          ...
                          padding: "2px 8px",
                          borderRadius: 20,
                          background: customerReturnCount >= 3 ? "#FEE2E2" : "#F3F4F6",
                          color: customerReturnCount >= 3 ? "#DC2626" : "#374151",
                          fontWeight: 600,
                        }}
                      >
                      ...
                      {(cCity || cCountry) && (
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <span style={{ fontSize: 13, color: "#374151" }}>
                            {[cCity, cCountry].filter(Boolean).join(", ")}
                          </span>
                        </div>
                      )}
                      {hasFullAddress && (
                        ...
                        >
                          ...
                          <div style={{ fontSize: 13, lineHeight: 1.6, color: "#374151" }}>
                            ...
                            {(cCity || cProvince || cZip) && (
                              <div>{[cCity, cProvince, cZip].filter(Boolean).join(", ")}</div>
                            )}
                  ...
                  {pickupAddress &&
                    (pickupAddress.formatted || pickupAddress.address1) &&
                    !hasFullAddress && (
                      <div
                      ...
                      >
                        ...
                        <div
                          style={{ fontSize: 13, lineHeight: 1.6, color: "#374151", marginTop: 4 }}
                        >
                          {pickupAddress.formatted ??
                            [
                              pickupAddress.name,
                  ...
                  <div style={{ marginTop: 12 }}>
                    <button
                      type="button"
                      onClick={() => setShowEditAddress((v) => !v)}
                      style={{
                        fontSize: 12,
                        color: "#2563EB",
                        background: "none",
                    ...
                    {showEditAddress && (
                      <fetcher.Form
                        ...
                        action={`/api/returns/${returnCase.id}/actions`}
                        style={{ marginTop: 10 }}
                        onSubmit={() => setShowEditAddress(false)}
                      >
                        ...
                        >
                          ...
                          <div>
                            ...
                            <input
                              ...
                              type="text"
                              name="customerCity"
                              defaultValue={returnCase.customerCity ?? ""}
                              maxLength={100}
                              style={{
                                width: "100%",
                                padding: "6px 8px",
                          ...
                          <div>
                            ...
                            <input
                              ...
                              defaultValue={returnCase.customerCountry ?? ""}
                              maxLength={100}
                              style={{
                                width: "100%",
                                padding: "6px 8px",
            ...
            {(returnCase as { isGiftReturn?: boolean }).isGiftReturn &&
              (() => {
                const gr = returnCase as {
                  giftRecipientName?: string | null;
                  giftRecipientEmail?: string | null;
                  giftMessageToSender?: string | null;
                };
                return (
                  <div style={{ ...C.card, borderColor: "#C4B5FD" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {gr.giftRecipientName && (
                        <div style={{ fontSize: 13 }}>
                          <strong>Name:</strong> {gr.giftRecipientName}
                        </div>
                      )}
                      {gr.giftRecipientEmail && (
                        <div style={{ fontSize: 13 }}>
                          <strong>Email:</strong> {gr.giftRecipientEmail}
                        </div>
                      )}
            ...
            <div style={{ ...C.card }}>
              ...
              {(() => {
                const mediaJson = (returnCase as { customerMediaJson?: string | null })
                  .customerMediaJson;
              ...
              >
                ...
                <textarea
                  ...
                  defaultValue={
                    (returnCase as { notesForCustomer?: string | null }).notesForCustomer ?? ""
                  }
                  rows={2}
                  placeholder="e.g. Please ship the item to..."
                  style={{
                    width: "100%",
                    padding: 8,
                    borderRadius: 6,
...
export function ErrorBoundary() {
  ...
  const heading = is404 ? "Return not found" : is400 ? "Invalid request" : "Something went wrong";
  ...
  const description = is404
    ...
    : is400
      ? typeof errorMessage === "string"
        ? errorMessage
        : "The request was invalid. Please go back and try again."
      : is500
