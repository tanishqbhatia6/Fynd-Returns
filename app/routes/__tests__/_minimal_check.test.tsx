...
vi.mock("../lib/shopify-admin.server", () => ({
  ...
  withRestCredentials: vi.fn((a) => a),
}));
vi.mock("../lib/return-request-id", () => ({
  parseReturnIdConfig: vi.fn(() => ({})),
  buildReturnRequestId: vi.fn(() => "X"),
  formatReturnRequestId: vi.fn((id) => `R-${id}`),
}));
vi.mock("../lib/return-id-counter.server", () => ({ nextReturnIdCounter: vi.fn() }));
vi.mock("../lib/fynd-payload.server", () => ({
  parseFyndPayloadForDisplay: vi.fn(),
  parseFyndOrderDetailsForTab: vi.fn(),
  getPickupAddressFromFyndPayload: vi.fn(),
  extractFyndJourney: vi.fn(() => []),
  extractCustomerFromFyndPayload: vi.fn(),
  extractShippingDetailsFromFyndPayload: vi.fn(),
  extractAffiliateOrderIdFromFyndPayload: vi.fn(),
  isLikelyFyndId: vi.fn(),
  buildTrackingUrlFromCourierAndAwb: vi.fn(),
}));
vi.mock("../lib/fynd.server", () => ({
  isFyndPrivateUrl: vi.fn(),
  signFyndUrl: vi.fn(),
...
describe("debug", () => {
  it("renders", async () => {
    ...
    const { container } = renderWithRouter(Comp, {
      initialEntries: ["/x"],
      loaderData: ld as never,
    });
