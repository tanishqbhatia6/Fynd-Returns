...
import { returnActionCounter, returnActionDuration, appErrorCounter, returnsRejectedCounter } from "../observability/metrics.server";
...
export const handleReject: ReturnActionHandler = async (ctx, body) => {
  const { id, returnCase, shop, admin, isTerminal, sessionEmail, shopDomain, elapsed, logShopifyReturnEvent } = ctx;
