...
export const handleRefreshFyndDetails: ReturnActionHandler = async (ctx) => {
  ...
  return await withSpan(
    "return.action.refresh_fynd_details",
    {
    ...
    async () => {
      ...
      try {
        ...
        if (!externalOrderId || returnCase.shopifyOrderId?.startsWith("manual:")) {
          ...
          throw redirect(
            `/app/returns/${id}?fyndError=${encodeURIComponent("No order number. Refresh from Fynd requires a valid order number.")}`,
          );
        }
        ...
        if (!("searchShipmentsByExternalOrderId" in fyndClient)) {
          ...
          returnActionDuration.record(actionTimer(), { action: "refresh_fynd_details" });
          throw redirect(
            `/app/returns/${id}?fyndError=${encodeURIComponent("Refresh from Fynd requires Platform API. Configure in Settings → Integrations.")}`,
          );
        }
        try {
          ...
          if (!Array.isArray(items) || items.length === 0) {
            throw new Error(
              `No shipments found for order ${externalOrderId}. Check order number and date range.`,
            );
          }
          let payload: unknown = searchRes;
          const fyndOrderId =
            (searchRes as { orderId?: string; shipmentId?: string }).orderId ??
            (searchRes as { orderId?: string; shipmentId?: string }).shipmentId ??
            null;
            ...
            try {
              ...
              if (fullShipments != null) {
                ...
                const fullList = Array.isArray(fullShipments)
                  ? fullShipments
                  : ((fullShipments as { items?: unknown[] })?.items ??
                    (fullShipments as { shipments?: unknown[] })?.shipments ??
                    []);
                /* v8 ignore stop */
          ...
          if (returnShipment) {
            const dp = (returnShipment.delivery_partner_details ??
            ...
            const rCarrier =
              String(
                dp.display_name ?? dp.name ?? returnShipment.dp_name ?? meta.cp_name ?? "",
              ).trim() || null;
            ...
            const rLabelUrl = inv
              ? String(inv.label_url ?? invLinks.label ?? "").trim() || null
              : null;
            // defensive nested invoice URL fallbacks
            /* v8 ignore start */
            const rInvoiceUrl = inv
              ? String(inv.invoice_url ?? invLinks.invoice_a4 ?? "").trim() || null
              : null;
            /* v8 ignore stop */
          ...
          await prisma.returnCase.update({
            ...
            /* v8 ignore start */
            data: {
              fyndPayloadJson: payloadJson ?? undefined,
