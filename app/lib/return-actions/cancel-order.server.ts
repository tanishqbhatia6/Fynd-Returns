    ...
    async () => {
      ...
      try {
        ...
        if (!returnCase.shopifyOrderId || returnCase.shopifyOrderId.startsWith("manual:")) {
          returnActionCounter.add(1, { action: "cancel_order", outcome: "error" });
          return Response.json(
            { error: "Cannot cancel: no valid Shopify order linked" },
            { status: 400 },
          );
        }
        ...
        if (!orderGid.startsWith("gid://")) {
          if (/^\d+$/.test(orderGid)) {
            ...
            if (!orderByName?.id) {
              returnActionCounter.add(1, { action: "cancel_order", outcome: "error" });
              return Response.json(
                { error: "Could not resolve Shopify order for cancellation" },
                { status: 400 },
              );
            }
