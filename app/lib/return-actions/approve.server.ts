      ...
      try {
        if (isTerminal) {
          ...
          return Response.json(
            { error: `Cannot approve: return is already ${returnCase.status}` },
            { status: 400 },
          );
        }
        ...
        if (consolidateEnabled && !isGreenReturn) {
          ...
          const resolvedType =
            bodyResolutionType && validResolutionTypes.includes(bodyResolutionType)
              ? bodyResolutionType
              : "refund";
          ...
          if (canCreateConsReturn) {
            try {
              ...
              refundLogger.warn(
                { err },
                "[Approve:consolidation] Shopify Return creation crashed (non-fatal)",
              );
            }
          }
          ...
          addBusinessEvent("return.approved", {
            "return.id": returnCase.id,
            "resolution.type": resolvedType,
            consolidation: true,
          });
          ...
          auditReturnAction(
            ...
            { status: { from: returnCase.status, to: "approved" } },
          );
        ...
        if (isGreenReturn) {
          refundLogger.info(
            { returnId: id },
            "[Approve] Green return — skipping Fynd sync (no shipment needed)",
          );
        } else {
          ...
          const fyndClientResult = settingsForApprove
            ...
            : {
                ok: false as const,
                error:
                  "Fynd is not configured. Go to Settings → Integrations and connect Fynd with Platform API to create returns on Fynd.",
              };
          if (fyndClientResult.ok && "getShipments" in fyndClientResult.client) {
            ...
            if (!returnCase.shopifyOrderId?.startsWith("manual:")) {
              try {
                ...
                refundLogger.warn(
                  { err: orderFetchErr },
                  "[Approve] Order fetch for affiliateOrderId failed (non-fatal)",
                );
              }
            }
            ...
            try {
              ...
              if (
                fyndResult.success &&
                (fyndResult.fyndReturnId ?? fyndResult.fyndShipmentId ?? fyndResult.alreadyExists)
              ) {
                ...
                addBusinessEvent("return.fynd_sync_completed", {
                  "return.id": returnCase.id,
                  "fynd.return_id": fyndReturnId || "",
                  duration_ms: fyndSyncDurationMs,
                });
                /* v8 ignore stop */
                ...
                refundLogger.warn(
                  { error: fyndResult.error },
                  "[Approve] Fynd create return failed",
                );
              }
            } catch (err) {
              fyndSyncDurationMs = Date.now() - syncStartTime;
            ...
            fyndError =
              "Fynd return creation requires Platform API (Company ID + Client ID/Secret). Configure in Settings → Integrations.";
          }
        }
        if (!isGreenReturn && !fyndReturnId && !fyndError) {
          fyndError =
            "Fynd sync completed but did not return a return ID. The return may have been created — check the Fynd dashboard, or retry.";
          isTransientFyndError = true;
        }
        ...
        const retryNextTime =
          fyndError && isTransientFyndError ? new Date(Date.now() + 2 * 60_000) : undefined;
        ...
        const updateResult = await prisma.returnCase.updateMany({
          ...
          data: {
            ...
            fyndSyncStatus: fyndReturnId
              ? "synced"
              : fyndError
                ? isTransientFyndError
                  ? "retry_scheduled"
                  : "failed"
                : undefined,
            fyndSyncError: fyndError || null,
            ...(fyndError && isTransientFyndError
        ...
        if (shouldCreateShopifyReturn) {
          try {
            ...
            if (shopifyReturnResult.success && shopifyReturnResult.shopifyReturnId) {
              ...
              refundLogger.warn(
                { error: shopifyReturnResult.error },
                "[Approve] Shopify Return creation failed (non-fatal)",
              );
              await prisma.returnEvent
                .create({
                  data: {
