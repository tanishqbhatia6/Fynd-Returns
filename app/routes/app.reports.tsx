...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    const [refundedCasesForRevenue, topProductsByReturns, customerReturnFrequency] =
      await Promise.all([
        prisma.returnCase.findMany({
    ...
    const topProductsData = topProductsCombined.sort((a, b) => b.count - a.count).slice(0, 10);
    ...
    /* v8 ignore start */
    const resolvedCount =
      (resolutionMap.refund ?? 0) +
      (resolutionMap.exchange ?? 0) +
      (resolutionMap.store_credit ?? 0) +
      (resolutionMap.replacement ?? 0);
    const exchangeConversionRate =
      resolvedCount > 0 ? Math.round(((resolutionMap.exchange ?? 0) / resolvedCount) * 100) : 0;
    /* v8 ignore stop */
    const revenueRetainedRate =
      revenueRetained + totalRefundAmount > 0
        ? Math.round((revenueRetained / (revenueRetained + totalRefundAmount)) * 100)
        : 0;
    ...
    const repeatReturnerRate =
      uniqueCustomerCount > 0 ? Math.round((repeatCustomerCount / uniqueCustomerCount) * 100) : 0;
    ...
    const createdByChannelData = channelCreatedBy.map((r) => ({
      channel: String(r.createdByChannel),
      count: r._count,
    }));
    const sourceChannelData = channelSource.map((r) => ({
      channel: String(r.sourceChannel),
      count: r._count,
    }));
    ...
    try {
      ...
      /* LATERAL not supported or no data — skip */
    }
...
export default function Reports() {
  ...
  const approvalRateDisplay =
    totalReturns > 0 ? fmtRate((approvedCount / totalReturns) * 100) : "0";
  const rejectionRateDisplay =
    totalReturns > 0 ? fmtRate((rejectedCount / totalReturns) * 100) : "0";
  const refundRateDisplay =
    approvedCount > 0 ? fmtRate((refundedCount / approvedCount) * 100) : "0";
  const fyndSyncRateDisplay =
    approvedCount > 0 ? fmtRate((fyndSyncedCount / approvedCount) * 100) : "0";
      ...
      <div className="app-content layout-wide" style={{ paddingBottom: 48 }}>
        ...
        <div className="dashboard-stat-grid mb-md">
          <div
            ...
            style={{ "--kpi-accent": "#14B8A6" } as React.CSSProperties}
          >
            <div className="kpi-header">
              <span className="kpi-icon">
                <svg
            ...
            <div className="kpi-meta">
              {resolutionChartData.find((d) => d.name === "Exchange")?.value ?? 0} of{" "}
              {resolvedCount} resolved
            </div>
            <div className="kpi-progress">
              <span style={{ width: `${Math.min(100, exchangeConversionRate)}%` }} />
            </div>
          </div>
          ...
          >
            ...
            <div className="kpi-progress">
              <span style={{ width: `${Math.min(100, revenueRetainedRate)}%` }} />
            </div>
          </div>

          <div
            className="dashboard-stat-card"
            style={{ "--kpi-accent": "#F97316" } as React.CSSProperties}
          >
            <div className="kpi-header">
              <span className="kpi-icon">
                <svg
            ...
            <div className="kpi-meta">
              {repeatCustomerCount} of {uniqueCustomerCount} ≥ 2 returns
            </div>
          </div>
          ...
          <div
            className="dashboard-stat-card"
            style={
              { "--kpi-accent": fraudAlertCount > 0 ? "#DC2626" : "#94A3B8" } as React.CSSProperties
            }
          >
            <div className="kpi-header">
              <span className="kpi-icon">
            ...
            <div
              className="kpi-value"
              style={{ color: fraudAlertCount > 0 ? "#B91C1C" : "#94A3B8" }}
            >
              {fraudAlertCount}
            </div>
                        ...
                        >
                          <span
                            style={{
                              ...
                              height: 8,
                              borderRadius: 2,
                              background: getStatusColor(d.name.toLowerCase()),
                              flexShrink: 0,
                            }}
                          />
                          <span style={{ color: "var(--rpm-text-muted)" }}>{d.name}</span>
                          <span style={{ fontWeight: 700 }}>{d.value}</span>
                          <span style={{ color: "var(--rpm-text-muted)", fontSize: 11 }}>
                            ({pct}%)
                          </span>
                        </div>
                      );
            ...
            {[
              {
                label: "Approval",
                value: approvalRate,
                display: approvalRateDisplay,
                color: "#10B981",
                desc: `${approvedCount} of ${totalReturns}`,
              },
              {
                label: "Rejection",
                value: rejectionRate,
                display: rejectionRateDisplay,
                color: "#EF4444",
                desc: `${rejectedCount} of ${totalReturns}`,
              },
              {
                label: "Refund",
                value: refundRate,
                display: refundRateDisplay,
                color: "#8B5CF6",
                desc: `${refundedCount} of ${approvedCount} approved`,
              },
              ...(hasFyndConfig
                ? [
                    {
                      label: "Fynd Sync",
                      value: fyndSyncRate,
                      display: fyndSyncRateDisplay,
                      color: "#06B6D4",
                      desc: `${fyndSyncedCount} of ${approvedCount}`,
                    },
                  ]
                : []),
            ].map((g, i) => (
                    ...
                    <PieChart>
                      ...
                      <Tooltip
                        contentStyle={{
                          ...
                          border: "1px solid #E2E8F0",
                          fontSize: 12,
                          boxShadow: "0 6px 16px -3px rgba(15,23,42,0.1)",
                        }}
                        formatter={
                          ((
                            value: number | undefined,
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            >
                              <span
                                style={{ fontSize: 13, fontWeight: 600, color: "var(--rpm-text)" }}
                              >
                                {d.name}
                              </span>
                              <span
                              ...
                              >
                                <strong style={{ color: "var(--rpm-text)", fontWeight: 700 }}>
                                  {d.value}
                                </strong>
                                {" · "}
                                {pct}%
              ...
              <div style={{ borderTop: "1px solid #E2E8F0", paddingTop: 16 }}>
                ...
                <div className="kpi-row">
                  ...
                  <span className="kpi-meta">
                    {totalReturns > 0
                      ? `${Math.round((greenReturnCount / totalReturns) * 100)}% of total`
                      : ""}
                  </span>
                </div>
                <div className="kpi-meta" style={{ marginTop: 4 }}>
        ...
        <div className="dashboard-chart-row mb-md">
          <div className={CS}>
            <div className="panel-header">
              ...
              <Link to="/app/settings/rules" className="panel-link">
                Manage →
              </Link>
            </div>
            {topReasons.length > 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {approvalRate >= 80 && (
                <div className="dashboard-suggestion dashboard-suggestion--success">
                  <strong>High approval rate ({approvalRate}%)</strong> — Return policy is
                  well-calibrated.
                </div>
              )}
              {approvalRate > 0 && approvalRate < 50 && (
                <div className="dashboard-suggestion dashboard-suggestion--warning">
                  <strong>Low approval rate ({approvalRate}%)</strong> — Review return policy to
                  improve customer satisfaction.
                </div>
              )}
              {avgProcessingDays !== null && avgProcessingDays > 3 && (
                <div className="dashboard-suggestion dashboard-suggestion--warning">
                  <strong>Avg processing: {avgProcessingDays.toFixed(1)} days</strong> — Consider
                  faster approvals for better retention.
                </div>
              )}
              {avgProcessingDays !== null && avgProcessingDays <= 1 && (
                <div className="dashboard-suggestion dashboard-suggestion--success">
                  <strong>Fast processing ({avgProcessingDays.toFixed(1)}d)</strong> — Returns are
              ...
              {approvedNotRefundedCount > 0 && (
                <div className="dashboard-suggestion dashboard-suggestion--info">
                  <strong>
                    {approvedNotRefundedCount} approved return
                    {approvedNotRefundedCount > 1 ? "s" : ""} awaiting refund
                  </strong>{" "}
                  — Process refunds to complete the cycle.
                </div>
              )}
              {topReasons.length > 0 && topReasons[0].count >= 2 && (
                <div className="dashboard-suggestion dashboard-suggestion--info">
                  <strong>Top reason: &ldquo;{topReasons[0].reason}&rdquo;</strong> (
                  {topReasons[0].count}x) — Investigate potential product or description issue.
                </div>
              )}
              {periodChange > 50 && (
                <div className="dashboard-suggestion dashboard-suggestion--warning">
                  <strong>Returns up {periodChange}%</strong> vs previous period — Monitor for
              ...
              {periodChange < -20 && (
                <div className="dashboard-suggestion dashboard-suggestion--success">
                  <strong>Returns down {Math.abs(periodChange)}%</strong> — Return rate is
                  decreasing.
                </div>
              )}
              {/* v8 ignore stop */}
        ...
        {(totalRefundAmount > 0 || refundMethodBreakdown.length > 0) && (
          <div
            className={refundMethodBreakdown.length > 0 ? "dashboard-chart-row" : ""}
            style={{ marginTop: 20 }}
          >
            <div className={CS}>
              <h3 className="panel-title" style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {refundMethodBreakdown.map((item) => (
                    <div key={item.method} className="flex-between">
                      <span className="kpi-meta" style={{ textTransform: "capitalize" }}>
              ...
              {topProductsData.map((item, idx) => {
                ...
                return (
                  <div key={idx} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span
                      className="text-muted text-tabular"
                      style={{ fontSize: 11, fontWeight: 700, width: 18, textAlign: "right" }}
                    >
                      {idx + 1}
                    </span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div className="flex-between" style={{ marginBottom: 2 }}>
                        <span
                          ...
                          style={{ fontSize: 12, fontWeight: 500, maxWidth: "70%" }}
                        >
                          {item.title}
                        </span>
                        <span className="text-tabular" style={{ fontSize: 12, fontWeight: 700 }}>
                          {item.count}
                        </span>
                      </div>
        ...
        {customerFrequencyData.length > 0 && customerFrequencyData[0].count >= 2 && (
          <div className={CS} style={{ marginTop: 20 }}>
            ...
            <div className="kpi-meta" style={{ marginBottom: 12 }}>
              Customers with the highest return counts in this period
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {customerFrequencyData
                .filter((c) => c.count >= 2)
                .map((item, idx) => (
                  <div
                    ...
                    style={{
                      padding: "6px 10px",
                      borderRadius: 8,
                      background: item.count >= 3 ? "#FEF2F2" : "#F9FAFB",
                    }}
                  >
                    <span
                      style={{
                        fontSize: 12,
                        color: item.count >= 3 ? "#DC2626" : "var(--rpm-text-muted)",
                      }}
                    >
                      {item.email}
                    </span>
                    <span
                      ...
                      style={{
                        color: item.count >= 3 ? "#DC2626" : "#374151",
                        background: item.count >= 3 ? "#FEE2E2" : "#E5E7EB",
                      }}
                    >
                      {item.count} returns
                    </span>
                ...
                return (
                  <div key={idx}>
                    <div className="flex-between" style={{ marginBottom: 2, fontSize: 13 }}>
                      ...
                      <span className="text-tabular" style={{ fontWeight: 700 }}>
                        {item.count}
                      </span>
                    </div>
                    <div style={{ height: 5, borderRadius: 3, background: "#E5E7EB" }}>
                  ...
                  {createdByChannelData.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex-between"
                        style={{ padding: "6px 10px", borderRadius: 8, background: "#F9FAFB" }}
                      >
                        <span
                          style={{ fontSize: 12, fontWeight: 500, textTransform: "capitalize" }}
                        >
                          {item.channel}
                        </span>
                        <span className="text-tabular" style={{ fontSize: 12, fontWeight: 700 }}>
        ...
        <div className="settings-summary-bar" style={{ marginTop: 20, justifyContent: "center" }}>
          <span>
            <strong>{allTimeReturns}</strong> total returns (all time)
          </span>
          <span>·</span>
          <span>
            <strong>{itemsCount}</strong> items returned ({rangeLabel})
          </span>
          <span>·</span>
          <span>
            ~<strong>{avgItemsPerReturn}</strong> items per return
          </span>
        </div>
      </div>
    </AppPage>
