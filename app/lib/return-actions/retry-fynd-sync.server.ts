...
import { returnActionCounter, returnActionDuration, appErrorCounter, fyndSyncCounter } from "../observability/metrics.server";
...
export const handleRetryFyndSync: ReturnActionHandler = async (ctx) => {
  ...
  return await withSpan("return.action.retry_fynd_sync", {
    ...
    try {
      ...
      const settingsRetry = shop.settings as (NonNullable<unknown> & { fyndApiType?: string | null }) | undefined;
