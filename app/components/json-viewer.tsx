      ...
      return (
        <>
          ...
          {open && (
            ...
            >
              {value.map((item, i) => (
                <JsonNode key={i} k={String(i)} value={item} depth={depth + 1} />
              ))}
            </div>
          )}
        </>
      ...
      return (
        <>
          ...
          {open && (
            ...
            >
              {entries.map(([ek, ev]) => (
                <JsonNode key={ek} k={ek} value={ev} depth={depth + 1} />
              ))}
            </div>
          )}
        </>
...
export function PayloadViewer({
  ...
  if (!rawPayload)
    return <div style={{ padding: 10, color: "#6B7280", fontSize: 12 }}>No payload</div>;
        ...
        {mode === "tree" && isValid ? (
          ...
          >
            ...
            {!isValid && (
              ...
              >
                Payload was truncated — showing raw text. New webhooks will capture the full
                payload.
              </div>
            )}
          </pre>
